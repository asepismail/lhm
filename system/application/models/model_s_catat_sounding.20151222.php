<?php
class model_s_catat_sounding extends Model{
    public $snd_type;
    ...
    function LoadData($company){
        ...
        if (!empty($sound_type) || trim($sound_type)!==''){
            $queries = "SELECT * FROM s_sounding WHERE ACTIVE=1 AND COMPANY_CODE='".$company."' AND TYPE_S='".$sound_type."' ORDER BY DATE DESC";    
        }else{
             $queries = "SELECT * FROM s_sounding WHERE ACTIVE=1 AND COMPANY_CODE='".$company."' ORDER BY DATE DESC";
        }
        ...
        foreach($objects as $obj)
        {
            ...
            array_push($cell, htmlentities(number_format($obj->VOLUME,3),ENT_QUOTES,'UTF-8'));
            array_push($cell, htmlentities(number_format($obj->WEIGHT,3),ENT_QUOTES,'UTF-8'));
    ...
    function add_new($company, $data_post){
        ...
        $cek_data_exist = $this->cek_data_exist('s_sounding',array('ID_SOUNDING'=>$data_post['ID_SOUNDING']),'ID_SOUNDING');
        if ($cek_data_exist > 0){
        ...
        if(empty($status) || $status===FALSE){
            $this->db->insert( 's_sounding', $data_post );
            if($this->db->trans_status() === FALSE){
                ...
                $this->calc_weight($data_post['TEMPERATURE'],$data_post['HEIGHT'],$data_post['ID_SOUNDING'],$data_post['ID_STORAGE'],$company);//hitung berat tangki  
            }
        }
        return $status;
    }
    ...
    function update_sounding($id_sounding,$data_post,$company){
        ...
        if(empty($status) || $status===FALSE){
            ...
            $this->db->update('s_sounding',$data_post);
            if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
                $this->calc_volume($data_post['HEIGHT'],$id_sounding,$data_post['ID_STORAGE'],$company);
                $this->calc_weight($data_post['TEMPERATURE'],$data_post['HEIGHT'],$id_sounding,$data_post['ID_STORAGE'],$company);
    ...
    function calc_weight($temp,$height,$id_sounding,$id_storage,$company){ //TONASE
        ...
        //$query="SELECT HEIGHT FROM s_sounding WHERE ID_SOUNDING='".$id_sounding."' AND COMPANY_CODE='".$company."'";
        ...
        if($company=='LIH'){
            ...
            $tonase=$volume_3*$bj; 			 
        }elseif($company=='GKM'|| $company=='SMI'){
			$hi=$height*1000;
			if ($hi>24 && $hi<100){
			...
			var_dump("const_d =".$const_d);	
			var_dump("bj_koreksi =".$bj_koreksi);			
			$tonase=$const_e*$bj;			
			var_dump("const_e =".$const_e);
			var_dump("bj =".$bj);
		}elseif($company=='MIA' || $company=='MAG'){
            $halaman=0;
            $cincin=0;
            ...
            /*
            $query="SELECT * FROM storage_volumefraksi_converter WHERE ID_STORAGE='".$id_storage."' AND CINCIN='".$cincin."'";
            $sQuery = $this->db->query($query);
            if ($sQuery->num_rows() > 0){
               $row = $sQuery->row();
               if ($halaman==1){
