...
function reloadGrid(){   
     ...
     jQuery("#list_ffa").setGridParam({url:url+'s_catat_ffa_prod/LoadData/'}).trigger("reloadGrid");    
}
...
colModelT.push({name:'ID_FFA_PROD',index:'ID_FFA_PROD', editable: false, hidden:true, width: 140, align:'left'});
...
var loadView = function()
        ...
        jGrid_va = jQuery("#list_ffa").jqGrid(
            {
                url:url+'s_catat_ffa_prod/LoadData/',
                mtype : "POST",
                datatype: "json",
                colNames: colNamesT ,
                colModel: colModelT,
...
function simpan_ffa(){
    ...
    if (answer){
        ...
        $.ajax({
                type:           'post',
                cache:          false,
                url:            url+'s_catat_ffa_prod/CRUD_METHOD',
                data:           {myJson:  data} ,
...
function update_ffa(){
    ...
    if (answer){
        ...
        postdata_id['ID_FFA_PROD'] = $("#txt_ffaID").val();
        postdata_id['DATE'] = $("#txt_ffaDate").val();
        postdata_id['FFA'] = $("#txt_ffa").val();
        postdata_id['MOISTURE'] =  $("#txt_ffaMoisture").val();
        postdata_id['DIRT'] =  $("#txt_ffaDirt").val();
...
function edit_ffa(cl){
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
        ...
        $("#txt_ffaID").val(data.ID_FFA_PROD);
...
function hapus_ffa(cl){
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
        ...
        var answer = confirm ("Hapus FFA dengan ID : " + data.ID_FFA_PROD + " ?" );
        if (answer){
            $("#frm_load").dialog('open');
            var postdata_id = {};
    
            postdata_id['ID_FFA_PROD'] = data.ID_FFA_PROD;
...
jQuery(document).ready(function(){
   ...
   $("#frm_ffa").dialog({
    ...
    $("#search_form").dialog({
        ...
        open:function(){
            jQuery("#list_ffa").smartSearchPanel('#search_form', {dialog:{width: 530}},'s_catat_ffa_prod/search_data');
        }
    });
});
