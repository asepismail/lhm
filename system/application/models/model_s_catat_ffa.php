<?php
class model_s_catat_ffa extends Model{
    function __construct(){
        parent::__construct();
        $this->load->database();
    }
    
    function LoadData($kode_storage,$periode,$company){
        ...
        if(!empty($kode_storage)){
            if($kode_storage!='-'){
                $wherek =  " AND ID_STORAGE LIKE ='%".$kode_storage."%'";    
            }   
        }  
        $queries = "SELECT * FROM s_ffa WHERE ACTIVE=1 AND COMPANY_CODE='".$company."'".$wherep.$wherek;
        ...
        foreach($objects as $obj)
        {
            $cell = array();
            array_push($cell, $no); 
            array_push($cell, htmlentities($obj->ID_FFA,ENT_QUOTES,'UTF-8'));
    ...
    function data_search($data_search,$company){
        ...
        $queries = "SELECT * FROM s_ffa ".$where;
    ...
    function add_new($company, $data_post){
        ...
        $cek_data_exist = $this->cek_data_exist('s_ffa',array('ID_FFA'=>$data_post['ID_FFA']),'ID_FFA');
        if ($cek_data_exist > 0){
            $status='Data Input ID telah ada di database';
        }
        
        unset($cek_data_exist);
        $cek_data_exist = $this->cek_data_exist('s_ffa',array('ACTIVE'=>1, 'ID_STORAGE'=>$data_post['ID_STORAGE'],'DATE'=>$data_post['DATE'],'COMPANY_CODE'=>$company),'ID_FFA');
        if ($cek_data_exist > 0){
            $status='Data FFA untuk storage: '.$data_post['ID_STORAGE'].' hari ini telah ada di database';
        }
        ...
        if(empty($status) || $status===FALSE){
            $this->db->insert( 's_ffa', $data_post );
    ...
    function update_ffa($id_ffa,$data_post,$company){
        ...
        $cek_data_exist = $this->cek_data_exist('s_ffa',array('ID_FFA'=>$id_ffa),'ID_FFA');
        if ($cek_data_exist <= 0){
        ...
        if(empty($status) || $status===FALSE){
            ...
            $this->db->where('ID_FFA',$id_ffa);
            $this->db->where('COMPANY_CODE',$company);
            $this->db->update('s_ffa',$data_post);
            if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
