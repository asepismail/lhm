<?php
class m_kondisi_tanaman extends Controller
{
    private $data;
    function __construct(){
        parent::__construct();
        $this->load->model('model_m_kondisi_tanaman');
        $this->load->model('model_c_user_auth');  
        
        $this->load->library('form_validation');
        
        $this->lastmenu="m_kondisi_tanaman";
		$this->load->library('csvReader');
        $this->data = array();    
    }
    
    function index(){
        $view="info_m_kondisi_tanaman";
        
        //$data = array();
        $this->data['judul_header'] = "Monitoring Kondisi Tanaman";
        $this->data['js'] = "";
        ...
        $this->data['periode_kondisi_tanaman'] = $this->global_func->drop_date2('bulan_kondisi_tanaman','tahun_kondisi_tanaman','select');
    ...
    function LoadData(){
        ...
        echo json_encode($this->model_m_kondisi_tanaman->LoadData($bulan,$tahun,$company));   
    }
    /*########################## PENETAPAN PERIODE kondisi_tanaman ###############################
    ##################################################################################*/
    function set_kondisi_tanaman_periode(){
        $company = htmlentities($this->session->userdata('DCOMPANY'),ENT_QUOTES,'UTF-8');
        $periode = htmlentities($this->uri->segment('3'));
        
        $set_kondisi_tanaman_periode = $this->model_m_kondisi_tanaman->set_kondisi_tanaman_periode($periode,$company);
        echo $set_kondisi_tanaman_periode;
    }
    function get_kondisi_tanaman_periode(){
        $company = htmlentities($this->session->userdata('DCOMPANY'),ENT_QUOTES,'UTF-8');
        $kondisi_tanaman_periode = $this->model_m_kondisi_tanaman->get_kondisi_tanaman_periode($company);
        
        return $kondisi_tanaman_periode; 
    }
	...
	function do_import(){
		...
		if(!empty($_FILES[$fileElementName]['error'])){
			...
			foreach($result as $field){
				$boolean_error = false;
				$items = array();
				$data_lokasi = $this->model_m_kondisi_tanaman->lokasi_validate(substr($field['BLOK_TANAH'],0,2),$field['BLOK_TANAH'],$company); 					
				...
				$data_kondisi_tanaman = $this->model_m_kondisi_tanaman->get_kondisi_tanaman_blok($company, $field['BLOK_TANAH'], $today); 
				if($data_kondisi_tanaman>0){	
					$reterror = $reterror +1;
					$error .= "Baris " . $baris . "  Kondisi Tanaman pada lokasi ". $field['BLOK_TANAH']. " Periode " .$bulan."-".$tahun." sudah pernah diinput <br/>";
					$boolean_error = true;
				}
				...
				if ($boolean_error == false){
					$items = array(
							...
							'POKOK_KUNING' => $field['POKOK_KUNING'],
							'POKOK_KERDIL' => $field['POKOK_KERDIL'],
							'INPUT_BY' => trim(htmlentities($this->session->userdata('LOGINID'),ENT_QUOTES,'UTF-8')),								
							'COMPANY_CODE' => $company
							);
					$query = $this->db->insert('s_kondisi_tanaman',$items);
					$ret += (int)$this->db->affected_rows($query);
				}
					
					$baris++;
			} 
			$msg = $ret . " data monitoring Kondisi Tanaman blok BERHASIL diupload, dan ". $reterror. " data Kondisi Tanaman blok GAGAL diupload";	
    ...
    function search_data(){
        ...
        echo json_encode($this->model_m_kondisi_tanaman->LoadData($bulan,$tahun,$company,$afd,$block));       
    }
    
    function get_afdeling(){
        $company = htmlentities($this->session->userdata('DCOMPANY'),ENT_QUOTES,'UTF-8');
        $q = htmlentities($_REQUEST['q'],ENT_QUOTES,'UTF-8');
        $data_afd = $this->model_m_kondisi_tanaman->get_afdeling($company,$q);
    ...
    function get_block(){
        ...
        $data_afd = $this->model_m_kondisi_tanaman->get_block($company,$location_left,$q);
    ...
    function add_new($data_id){
        ...
        $data_post['POKOK_KUNING']  =strtoupper(trim(htmlentities($data_id['POKOK_KUNING'],ENT_QUOTES,'UTF-8'))) ;
		$data_post['POKOK_KERDIL']  =strtoupper(trim(htmlentities($data_id['POKOK_KERDIL'],ENT_QUOTES,'UTF-8'))) ;
        $data_post['INPUT_BY'] = htmlentities($this->session->userdata('LOGINID'),ENT_QUOTES,'UTF-8'); 
        $data_post['COMPANY_CODE'] = $company;

        $validate_numeric=$this->validate_numeric($data_post['POKOK_KUNING']);
        if( strtolower($validate_numeric)=='false'){
            $return['status']="Nilai POKOK_KUNING harus angka";
            $return['error']=true;        
        }
		
		$validate_numeric=$this->validate_numeric($data_post['POKOK_KERDIL']);
        if( strtolower($validate_numeric)=='false'){
            $return['status']="Nilai POKOK_KERDIL harus angka";
            $return['error']=true;        
        }
        ...
        $data_lokasi = $this->model_m_kondisi_tanaman->lokasi_validate($data_post['AFD'],$data_post['BLOK_TANAH'],$company);   
        ...
        if(empty($status)){     
            $insert_id = $this->model_m_kondisi_tanaman->add_new($company,$data_post);
    ...
    function update_data($data_id){
        ...
        $validate_numeric=$this->validate_numeric($data_post['POKOK_KUNING']);
        if( strtolower($validate_numeric)=='false'){
            $status ="Nilai POKOK_KUNING harus angka";
            $return['status']=$status;
            $return['error']=true;        
        }
		...
		$validate_numeric=$this->validate_numeric($data_post['POKOK_KERDIL']);
        if( strtolower($validate_numeric)=='false'){
            $status ="Nilai POKOK_KERDIL harus angka";
            $return['status']=$status;
            $return['error']=true;        
        }
        ...
        if(empty($status)){     
            $insert_id = $this->model_m_kondisi_tanaman->update_kondisi_tanaman($data_post, $company);
            $return['status']=  $insert_id;
            $return['error']=false;
            echo json_encode($return);
    ...
    function delete_data($data_id){
        ...
        $delete_kondisi_tanaman = $this->model_m_kondisi_tanaman->delete_kondisi_tanaman($afd,$block,$bulan,$tahun,$company);
        $return['status']=  $delete_kondisi_tanaman;
        $return['error']=false;
        echo json_encode($return);
    }
	...
	function delete_all_data($data_id){
		...
		foreach($data_detail as $key => $val){
			...
			}elseif (preg_match('/ID_KONDISI_TANAMAN/',$key)){
            	$data_post[$tmp_identifier]=array_merge((array)$data_post[$tmp_identifier],array('ID_KONDISI_TANAMAN'=>$val));             
			...
			}elseif (preg_match('/POKOK_KUNING/',$key)){
	        	$data_post[$tmp_identifier]=array_merge((array)$data_post[$tmp_identifier],array('POKOK_KUNING'=>$val)); 
			}elseif (preg_match('/POKOK_KERDIL/',$key)){
	        	$data_post[$tmp_identifier]=array_merge((array)$data_post[$tmp_identifier],array('POKOK_KERDIL'=>$val)); 
			}
        ...
        if(empty($return['status']) && $return['error']==false){    
            $insert_id = $this->model_m_kondisi_tanaman->delete_all_data($data_post);
            $return['status']=  $insert_id;
            $return['error']=false;
            echo json_encode($return);
    ...
    function search_data_2(){
        ...
        echo json_encode($this->model_m_kondisi_tanaman->data_search($data['rules'], $company));
    }
	...
	function xls_month(){
        ...
        $obj =& get_instance();
		$headers .= "PT. ".$company."\n";
		$headers .= "MONITORING KONDISI TANAMAN \n";
		...
		$headers .= "POKOK \t";	
		$headers .= "SPH \t";	
		$headers .= "POKOK KUNING \t";
		$headers .= "POKOK KERDIL\t";	
		
		$no = 1;
		$data=$this->model_m_kondisi_tanaman->get_xls($company_code, $bulan, $tahun);
		if($data!=NULL){
			foreach ($data as $row){
				$line = '';
				$line .= str_replace('"', '""',$no)."\t"; 
				...
				$line .= str_replace('"', '""',$row['POKOK_KUNING'])."\t";
				$line .= str_replace('"', '""',$row['POKOK_KERDIL'])."\t";
				$no++;
				$data .= trim($line)."\n"; 
        ...
        header("Content-Disposition: attachment; filename=HPT_kondisi_tanaman".$company."_".$periode.".xls");
        echo "$judul\n$headers\n$data";
		}
