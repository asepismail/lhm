...
class pms_m_pengajuan extends Model{
	...
	function insertAttchDB(){
		$this->db->insert( 'pms_project_proposal_attach', $data );
		return $this->db->insert_id();
	}
	
	function read_detail_attacment($pjs)
    {
        $limit = $this->input->post('rows');
        $page = $this->input->post('page');
        $sidx = $this->input->post('sidx');
        $sord = $this->input->post('sord');
	 	...
	 	$sql2 = " SELECT ATTACHMENT_ID, PROJECT_PROPNUM_NUMID, PROJECT_ID, ATTACHMENT_DESC, ATTACHMENT_LOCATION, INPUTBY, INPUTDATE FROM pms_project_proposal_attach";
		$sql2 .= " WHERE PROJECT_ID = '".$pjs."' ";
        ...
        foreach($objects as $obj){
            $cell = array();
                    array_push($cell, htmlentities($obj->PROJECT_PROPNUM_NUMID,ENT_QUOTES,'UTF-8'));
					array_push($cell, htmlentities($obj->PROJECT_ID,ENT_QUOTES,'UTF-8'));
					array_push($cell, htmlentities($obj->ATTACHMENT_DESC,ENT_QUOTES,'UTF-8'));
					array_push($cell, htmlentities($obj->ATTACHMENT_LOCATION,ENT_QUOTES,'UTF-8'));
					array_push($cell, htmlentities($obj->INPUTBY,ENT_QUOTES,'UTF-8'));
                    array_push($cell, htmlentities($obj->INPUTDATE,ENT_QUOTES,'UTF-8'));
                    array_push($cell, htmlentities($act,ENT_QUOTES,'UTF-8'));
            $row = new stdClass();
            $row->id = $cell[0];
            $row->cell = $cell;
