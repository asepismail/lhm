<?php
class model_m_kondisi_tanaman extends Model
{
    ...
    function LoadData($bulan,$tahun,$company,$afd='-',$block='-'){
        ...
        $queries = "SELECT * FROM s_kondisi_tanaman WHERE ACTIVE=1 AND COMPANY_CODE='".$company."' 
        ...
        foreach($objects as $obj)
        {
            $cell = array();
            array_push($cell, $no); 
            array_push($cell, htmlentities($obj->ID_KONDISI_TANAMAN,ENT_QUOTES,'UTF-8'));
            ...
            array_push($cell, htmlentities($obj->POKOK_KUNING,ENT_QUOTES,'UTF-8'));
			array_push($cell, htmlentities($obj->POKOK_KERDIL,ENT_QUOTES,'UTF-8'));
    ...
    function data_search($data_search, $company){
        ...
        $queries = "SELECT * FROM s_kondisi_tanaman ". $where;
    ...
    ##################################################################################*/
	/*
    function set_kondisi_tanaman_periode($periode,$company){
        $periode = $this->db->escape_str($periode);
        $company = $this->db->escape_str($company);
        
        $query = "SELECT PERIODE FROM s_kondisi_tanaman_periode WHERE COMPANY_CODE='".$company."'";
        ...
        if($sQuery->num_rows() > 0){
            ...
            $this->db->update('s_kondisi_tanaman_periode',$data);
            if($this->db->trans_status() === FALSE){
             ...
             $this->db->insert('s_kondisi_tanaman_periode');
             if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
	...
	*/
    /*
    function get_kondisi_tanaman_periode($company){
        $company = $this->db->escape_str($company);
        $query="SELECT PERIODE_USED FROM s_data_kondisi_tanaman WHERE COMPANY_CODE ='".$company."'";
        $sQuery = $this->db->query($query);
        $value='';
        if($sQuery->num_rows() > 0){
            $row = $sQuery->row();
            ...
            if(!empty($row->PERIODE_USED) || $row->PERIODE_USED!=null){
                ...
                $value = '<span style="color: #FF0000; font-weight: bold;"><em>kondisi_tanaman NOT SET</em></span>';   
            }
        }else{
            $value = '<span style="color: #FF0000; font-weight: bold;"><em>kondisi_tanaman NOT SET</em></span>';   
        } 
	...
	*/
    /*########################## END PENETAPAN PERIODE kondisi_tanaman ###############################
    ##################################################################################*/
    ...
    function add_new($company, $data_post){
        ...
        if( isset($company))
        {
            if(!empty($company))
            {
                $cek_data_exist = $this->cek_data_exist('s_kondisi_tanaman',array('AFD'=>$data_post['AFD'],'BLOK_TANAH'=>$data_post['BLOK_TANAH'], 'BULAN'=>$data_post['BULAN'],'TAHUN'=>$data_post['TAHUN'], 'COMPANY_CODE'=>$company,'ACTIVE'=>1),'ID_KONDISI_TANAMAN'); 
				 
				 
                if ($cek_data_exist <= 0){
                    $this->db->insert( 's_kondisi_tanaman', $data_post ); 
    ...
    function update_kondisi_tanaman($data_post, $company){
		$company = trim($this->db->escape_str($company));
        $status=FALSE;
        ...
        if(empty($status) || $status==false){
            ...
            $this->db->update('s_kondisi_tanaman',$data_post);
            if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
	...
	/*
		get_active_kondisi_tanaman added by Asep, 20130507		
	*/
	/*
	function get_active_kondisi_tanaman($company, $block, $tgl){
        $company = $this->db->escape_str($company);
        ...
        $query="SELECT bj.AFD,bj.BLOCK,bj.VALUE,
				...
				JOIN (
						...
						GROUP BY BLOCK 
				) kondisi_tanaman ON kondisi_tanaman.AFD = bj.AFD AND kondisi_tanaman.BLOCK = bj.BLOCK 
								AND kondisi_tanaman.MAX_PERIODE = bj.PERIODE
WHERE bj.BLOCK = '".$block."'";

		//$query="SELECT kondisi_tanaman.VALUE FROM s_data_kondisi_tanaman kondisi_tanaman WHERE kondisi_tanaman.BLOCK = '".$block."'  AND kondisi_tanaman.ACTIVE = 1 AND kondisi_tanaman.COMPANY_CODE ='".$company."'";
        $sQuery = $this->db->query($query);
        $value='';
        if($sQuery->num_rows() > 0){
            $row = $sQuery->row();            
	...
	*/
	function get_kondisi_tanaman_blok($company, $block, $tgl){
        $company = $this->db->escape_str($company);
		
        $query="SELECT * FROM s_kondisi_tanaman WHERE COMPANY_CODE='".$company."' AND CONCAT(TAHUN,BULAN) = DATE_FORMAT('".$tgl."','%Y%m') AND ACTIVE=1 AND BLOK_TANAH = '".$block."'";
	...
	function delete_all_data($data_post){
		...
		foreach($data_post as $keys => $vals){
			$this->db->where('ID_KONDISI_TANAMAN',$vals['ID_KONDISI_TANAMAN']);
			...
			$this->db->update('s_kondisi_tanaman',$data_post[$keys]);
    ...
    function delete_kondisi_tanaman($afd,$block,$bulan,$tahun,$company){
        $afd = $this->db->escape_str($afd);
        ...
        $status=FALSE;
        $cek_data = $this->cek_data_exist('s_kondisi_tanaman',
                    array('AFD'=>$afd,'BLOK_TANAH'=>$block,'BULAN'=>$bulan,'TAHUN'=>$tahun,
                            'COMPANY_CODE'=>$company),
                    'ID_KONDISI_TANAMAN');
        if ($cek_data <= 0){
            $status='Data kondisi_tanaman tidak ada di database';
        }
        ...
        if(empty($status) || $status==false){
            ...
            $this->db->set($set);
            $this->db->update( 's_kondisi_tanaman');
            
            //$this->db->delete('s_kondisi_tanaman');
            if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
...
,rawat.POKOK_KUNING, rawat.POKOK_KERDIL
FROM (	
...
LEFT JOIN (
	SELECT * FROM s_kondisi_tanaman r 
