...
colNamesT.push('TIPE KATAGORI');
colModelT.push({name:'TIPE_KATAGORI',index:'TIPE_KATAGORI', editable: true, hidden:false, width: 70, align:'right'});
...
function update_hpt(){
    ...
    postdata_id['AFD'] =  $("#txt_hptAFD").val();
    postdata_id['BLOCK'] =  $("#txt_hptBlock").val();
    postdata_id['VALUE'] = $("#txt_hptValue").val();
    ...
    $.ajax({
            type:           'post',
            cache:          false,
            url:            url+'m_hpt/CRUD_METHOD',
            data:           {myJson:  data} ,
            success: function(msg){
                var obj = jQuery.parseJSON(msg);    
                if(obj.error===true){
                    alert(obj.status)
                    $("#frm_hpt").dialog('close');
                }else{
                    alert(obj.status)
                    $("#frm_hpt").dialog('close');
                    reloadGrid();  
...
function delete_all(){
	...
	s = jQuery("#list_hpt").jqGrid('getGridParam','selarrrow');
	if (s == ""){
		alert ("Data Hama Penyakit tanaman yang akan di Approve belum dipilih");
	}else{
		postdata_id['CRUD'] =  'DELL_ALL';
		$.each(s, function(n, rowid){ 
			var str = jQuery("#list_hpt").jqGrid('getGridParam','selarrrow');
			//alert(str[i]);
			var data = $("#list_hpt").getRowData(str[i]) ;		
			i=i+1;
...
/*
function set_hpt_periode_do(){
    var answer = confirm ("Tetapkan Periode hpt ? " )
    if (answer){
        var answer_2 = confirm ("semua perhitungan hasil timbangan akan menggunakan referensi hpt periode: "+$("#tahun_hpt").val()+$("#bulan_hpt").val() )
        if (answer){
            var hpt_periode = $("#tahun_hpt").val()+$("#bulan_hpt").val();
            $.ajax({
                type:           'post',
                cache:          false,
                url:            url+'m_hpt/set_hpt_periode/'+hpt_periode,
                success: function(msg){
                    alert(msg);
                    $("#frm_hpt_set").dialog('close'); 
                }
            });
        }
