...
class m_hpt extends Controller
{
	...
	function do_import(){
		...
		if(!empty($_FILES[$fileElementName]['error'])){
			...
			foreach($result as $field){
				...
				if(empty($data_tipe)){						
					$reterror = $reterror +1;
					$error .= "Baris " . $baris . "  Tipe HPT ". $field['TIPE_HPT']. " pada blok ". $field['BLOK_TANAH']. " tidak ditemukan di data master <br/>";
					$boolean_error = true;
				}
				
				$data_hpt = $this->model_m_hpt->get_hpt_blok($company, $field['BLOK_TANAH'], $today); 
				if($data_hpt>0){	
					$reterror = $reterror +1;
					$error .= "Baris " . $baris . "  Hama Penyakit Tanaman pada lokasi ". $field['BLOK_TANAH']. " Periode " .$bulan."-".$tahun." sudah pernah diinput <br/>";
					$boolean_error = true;
				}
				
				if($field['TIPE_KATAGORI']>3){	
					$reterror = $reterror +1;
					$error .= "Baris " . $baris . "  Tipe Katagori ". $field['TIPE_KATAGORI']. " pada lokasi ". $field['BLOK_TANAH']. " TIDAK VALID. Tipe katagori yang diijinkan hanya 0=NIHIL, 1=RINGAN, 2=SEDANG, 3=BERAT<br/>";
					$boolean_error = true;
				}
				
				if($field['TIPE_KATAGORI']<0){	
					$reterror = $reterror +1;
					$error .= "Baris " . $baris . "  Tipe Katagori ". $field['TIPE_KATAGORI']. " pada lokasi ". $field['BLOK_TANAH']. " TIDAK VALID. Tipe katagori yang diijinkan hanya 0=NIHIL, 1=RINGAN, 2=SEDANG, 3=BERAT<br/>";
					$boolean_error = true;
				}
	...
	function get_HPTType(){
        ...
        foreach($data_afd as $row)
        {
            $block[] = '{res_id:"'.str_replace('"','\\"',htmlentities($row['HPT_CODE'],ENT_QUOTES,'UTF-8')).
                '",res_name:"'.str_replace('"','\\"',htmlentities($row['HPT_CODE'],ENT_QUOTES,'UTF-8')).
                '",res_dl:"'.str_replace('"','\\"',htmlentities($row['HPT_CODE'],ENT_QUOTES,'UTF-8'). "&nbsp;&nbsp; - &nbsp;&nbsp;" 
                .htmlentities($row['HPT_NAME'],ENT_QUOTES,'UTF-8')).'"}';
        }
        echo '['.implode(',',$block).']'; exit;     
    }
    ...
    function add_new($data_id){
		...
		$validate_numeric=$this->validate_numeric($data_post['TIPE_KATAGORI']);
        if( strtolower($validate_numeric)=='false'){
            $return['status']="Nilai TIPE_KATAGORI harus angka";
            $return['error']=true;        
        }
		
		if((int)$data_post['TIPE_KATAGORI'] > 3){
            $status  ="Tipe katagori yang diijinkan hanya 0=NIHIL, 1=RINGAN, 2=SEDANG, 3=BERAT";
            $return['status']=$status;
            $return['error']=true;
        }
		
		if((int)$data_post['TIPE_KATAGORI'] < 0){
            $status  ="Tipe katagori yang diijinkan hanya 0=NIHIL, 1=RINGAN, 2=SEDANG, 3=BERAT";
            $return['status']=$status;
            $return['error']=true;
        }
    ...
    function update_data($data_id){
		...
		$validate_numeric=$this->validate_numeric($data_post['TIPE_KATAGORI']);
        if( strtolower($validate_numeric)=='false'){
            $status ="Nilai TIPE_KATAGORI harus angka";
            $return['status']=$status;
            $return['error']=true;        
        }
	...
	function xls_month(){
		...
		if($data!=NULL){
			foreach ($data as $row){
				...
				if($row['TIPE_KATAGORI']==0){
					$tipe_katagori='NIHIL';
				}else if($row['TIPE_KATAGORI']==1){
					$tipe_katagori='RINGAN';
				}else if($row['TIPE_KATAGORI']==2){
					$tipe_katagori='SEDANG';
				}else if($row['TIPE_KATAGORI']==3){
					$tipe_katagori='BERAT';
				}else{
					$tipe_katagori='';
				}
