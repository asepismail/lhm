...
class Model_p_progress_tp extends Model 
{

    function Model_p_progress_tp()
    {
        parent::Model(); 
	...
	function insert_p_progress_tp ( $data )
	{
		$this->db->insert( 'p_progress_tp', $data );
		
		return $this->db->insert_id();   
	}
	
	function update_p_progress_tp ( $afd,$tgl_progress,$activity,$location,$company, $data )
	{
		...
		$this->db->update( 'p_progress_tp', $data );   
	}
...
AND ACTIVITY_CODE IN (SELECT ACCOUNTCODE FROM m_progress_map WHERE PENGGUNAAN = 'TP' AND PARENT = 0) 
...
GROUP BY LHM_DATE, LOCATION_CODE, ACTIVITY_CODE ORDER BY ".$sidx." ".$sord.""; 
		...
		foreach($objects as $obj)
        {
            $cell = array();
				...
				array_push($cell, $ID_PROGRESS_TP);
	...
	function cek_tp($tgl, $lc, $company){
		...
		$query = $this->db->query("select count(*) as jumlah FROM p_progress_tp where ".$where." AND DATE_FORMAT(TGL_PROGRESS, '%Y%m%d') = '".$tgl."' and COMPANY_CODE = '".$company."'");
	...
	function read_exist_act($tgl,$lc, $company)
    {
		...
		$sql2 = "SELECT ID_PROGRESS_TP, TGL_PROGRESS, LOCATION_CODE, ACTIVITY_CODE, ACTIVITY_DESC, SATUAN, HASIL_KERJA, COMPANY_CODE FROM p_progress_tp
WHERE ".$where." AND DATE_FORMAT(TGL_PROGRESS, '%Y%m%d') = '".$tgl."' AND COMPANY_CODE = '".$company."'";
		...
		$sql = "SELECT ID_PROGRESS_TP, TGL_PROGRESS, LOCATION_CODE, ACTIVITY_CODE, ACTIVITY_DESC, SATUAN, 
	HASIL_KERJA, COMPANY_CODE  FROM p_progress_tp
WHERE ".$where." AND DATE_FORMAT(TGL_PROGRESS, '%Y%m%d') = '".$tgl."' AND COMPANY_CODE = '".$company."' 
ORDER BY ".$sidx." ".$sord.""; 
		...
		foreach($objects as $obj)
        {
            $cell = array();
				...
				array_push($cell, $obj->ID_PROGRESS_TP);
...
WHERE ACTIVITY_CODE IN (SELECT ACCOUNTCODE FROM m_progress_map WHERE PENGGUNAAN = 'TP' AND PARENT = 0) and LOCATION_CODE like '".$q."%' AND COMPANY_CODE = '".$company."' AND DATE_FORMAT(LHM_DATE,'%Y%m') = '".$periode."' order by LOCATION_CODE asc");
		$temp_result = array();
		foreach ( $query->result_array() as $row )
		{
			$temp_result [] = $row;
