<?
class rpt_ba_pjbibitan extends Controller 
{
	function rpt_ba_pjbibitan ()
	{
		parent::Controller();	
        ...
        $this->lastmenu="rpt_ba_pjbibitan";
	...
	function js_ba_pjbibitan(){
		...
		$js = "$(function() {
			...
			var urlafd = url + 'rpt_ba_pjbibitan/ba_pjbibitan_rekap_afd/' + from + '/' + to + '/' + rkp; 
			var urlbl = url + 'rpt_ba_pjbibitan/ba_pjbibitan_rekap_block/' + from + '/' + to + '/' + rkp; 
			if(rkp == 'rekap')
			{
				urls = urlafd;
			} else {
			...
			else if ( jns_laporan == 'excell')
			{
				if(rkp == 'rekap')
				{
					urls = url + 'rpt_ba_pjbibitan/ba_xlspjbibitan_rekap_afd/' + from + '/' + to + '/' + rkp;
				} else {
					urls = url + 'rpt_ba_pjbibitan/ba_xlspjbibitan_rekap_block/' + from + '/' + to + '/' + rkp;
				}
				$.download(urls,'');
			}
			else if (jns_laporan =='pdf')
			{
				if(rkp == 'rekap')
				{
					urls = url + 'rpt_ba_pjbibitan/ba_pdfpjbibitan_rekap_afd/'  + from + '/' + to + '/' + rkp; 
				} else {
					urls = url + 'rpt_ba_pjbibitan/ba_pdfpjbibitan_petak/'  + from + '/' + to + '/' + rkp; 
				}
				$('#frame').attr('src',urls); 
	...
	function ba_pjbibitan_rekap_afd($periode){
		...
		$data_bibitan = $this->model_rpt_ba->ba_pjbibitan_afd($from, $to, $rkp, $company);
		...
		$tabel = "";
		$tabel .= $this->getStyle();
		$tabel .= "<table class='tbl_2' border='0' width='85%'><tr><td colspan='3' align='center'><strong>BERITA ACARA HASIL KERJA PROJECT PEMBIBITAN
					</strong></td>";
		$tabel .= "</tr><tr><td colspan='3' align='center'><strong>NO : &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; / PJ-BBT / ".$company." / ".$bulanr." / ".		$tahun."</strong></td> ";
 ...
 $tabel .= "<tr><th class='tbl_th'>BLN INI</th><th class='tbl_th'>s.d BLN INI</th><th class='tbl_th'>BLN INI</th><th class='tbl_th'>s.d BLN INI</th><th class='tbl_th'>BLN INI</th>
    ...
    <td class='tbl_td'><strong>&nbsp;&nbsp;PROJECT BIBITAN</strong></td>
    <td class='tbl_td' align = 'center'>Kg</td>
    <td class='tbl_td'>&nbsp;</td>
    <td class='tbl_td'>&nbsp;</td>
    <td class='tbl_td' align = 'right'><strong>".number_format($total,2,',','.')."</strong>&nbsp;&nbsp;</td>
    <td class='tbl_td' align = 'right'><strong>".number_format(0,2,',','.')."</strong>&nbsp;&nbsp;</td>
    <td class='tbl_td'>&nbsp;</td>
    <td class='tbl_td'>&nbsp;</td>
  </tr>";
		$tabel .= "</table>"; 
	...
	function ba_pjbibitan_rekap_block(){
		...
		$tabel .= "<table class='tbl_2' border='0'><tr><td colspan='3'>PT. ".$this->session->userdata('DCOMPANY_NAME')."</td>
					</tr><tr><td colspan='3'>REALISASI KERJA PROJECT PERSIAPAN PEMBIBITAN</td>";
		...
		$tabel .= "<tr><th rowspan='2' class='tbl_th'>KODE PROJECT</th><th rowspan='2' class='tbl_th'>BLOK / PETAK</th>
	...
	foreach ( $data_bibitan as $row){
		...
		$tabel .= "<td class='tbl_td' width='8%' align = 'center'>&nbsp;&nbsp;".$row['PJ_LOCATION']."</td>";
		...
		if($total > 0 && $total_hasilkerja > 0){ 
			$ttlhkunit = number_format($total/$total_hasilkerja,2,',','.');
		} else {
			$ttlhkunit = 0;
		}
		 
		$tabel .= "<td class='tbl_td' align = 'right'><strong>".$ttlhkunit."&nbsp;&nbsp;&nbsp;</td></tr>";
		$tabel .= "</table>"; 
		echo $tabel;
	}
	
	
	function ba_xlspjbibitan_rekap_afd() { 
		$from = $this->uri->segment(3);
		$to = $this->uri->segment(4);
		$rkp = $this->uri->segment(5);
		$company = $this->session->userdata('DCOMPANY');
		...
		$judul .= "REKAP REALISASI KERJA PROJECT PERSIAPAN PEMBIBITAN \n";
		$judul .= "NO :     / PJBBT / ".$company." / ".$bulanr." / ".$tahun."\n";
		...
		$footer .= str_replace('"', '""','-')."\t";
		$footer .= " PROJECT PERSIAPAN BIBITAN \t";
		$footer .= str_replace('"', '""','-')."\t";
		$footer .= str_replace('"', '""','-')."\t";
		$footer .= str_replace('"', '""','-')."\t";
        ...
        header("Content-Disposition: attachment; filename=BA_PJBIBITAN_".$company."_". substr($from,0,6) .".xls");
        echo "$judul\n$headers\n$data"; 
	}
	
	function ba_xlspjbibitan_rekap_block() { 
		$from = $this->uri->segment(3);
		$to = $this->uri->segment(4);
		$rkp = $this->uri->segment(5);
		$company = $this->session->userdata('DCOMPANY');
		...
		$judul .= $this->session->userdata('DCOMPANY_NAME'). "\n";
		$judul .= "REALISASI KERJA PROJECT PERSIAPAN PEMBIBITAN \n";
		$judul .= "PERIODE : ".strtoupper($bulan)."  " .$tahun."\n";
		$judul .= " \n";
		$judul .= "HK / HARI : " .$umr. " \n";
		...
		$footer .= str_replace('"', '""',$total)."\t";
		$footer .= str_replace('"', '""',$total/$total_hasilkerja)."\t";
				
		$data .= trim($footer)."\n";
		$data = str_replace("\r","",$data);
		header("Content-type: application/x-msdownload");
        header("Content-Disposition: attachment; filename=BA_PJBIBITAN_PERPETAK_".$company."_".substr($from,0,6).".xls");
        echo "$judul\n$headers\n$data";  
	}
	
	############################### PDF PRINT #########################
	function ba_pdfpjbibitan_rekap_afd()
	{
		...
		require_once(APPPATH . 'libraries/ba/header_ba_pjbibitan.inc');
		...
		$data_test=array();
		$data_test[1]['TEXT'] = "TOTAL BIAYA PROJECT PERSIAPAN BIBITAN";
	...
	function ba_pdfpjbibitan_petak(){
		if ($this->session->userdata('logged_in')!=TRUE){
			redirect('login');
		}
		$from = $this->uri->segment(3);
		...
		$total = 0; $total_hasilkerja = 0; $total_hk = 0; $total_hk_sat = 0; $total_hk_unit = 0; $total_qty = 0;
		...
		for($i=0; $i<=$columns; $i++) {
			...
			if($i == 1) {
				$aSimpleHeader[$i]['TEXT'] = "BLOK / PETAK";
				$aSimpleHeader[$i]['WIDTH'] = 25;
				$aSimpleHeader[$i]['ROWSPAN'] = 2;
				$aSimpleHeader[$i]['T_SIZE'] = 10;
				$aSimpleHeader[$i]['LN_SIZE'] = 5;
		...
		$data_test[8]['TEXT'] = number_format($total/$total_hasilkerja,2,',','.');
