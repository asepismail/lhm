<?php
class model_s_temperature_converter extends Model{
    ...
    function LoadData($company,$periode){
        ...
        $queries = "SELECT ID_STORAGE, TEMPERATURE, BJ, BJ_CORRECTION,COMPANY_CODE, ID_ANON 
                        FROM storage_temperature_converter WHERE COMPANY_CODE='".$company."'";
        ...
        foreach($objects as $obj){
            ...
            array_push($cell, htmlentities($obj->BJ,ENT_QUOTES,'UTF-8'));
            array_push($cell, htmlentities($obj->BJ_CORRECTION,ENT_QUOTES,'UTF-8'));
    ...
    function data_search($data_search,$company){
        ...
        $queries ="SELECT ID_STORAGE, TEMPERATURE, BJ, BJ_CORRECTION,COMPANY_CODE, ID_ANON 
                        FROM storage_temperature_converter ".$where;
    ...
    function add_new($tmp, $idstrg, $company, $data_post){
        ...
        $cek_data_exist = $this->cek_data_exist('storage_temperature_converter',
                    array('ID_STORAGE'=>$idstrg,'TEMPERATURE'=>$tmp),'ID_ANON');
        if ($cek_data_exist > 0){
            $status='Data Input Temperature telah ada di database';
        }
        
        if(empty($status) || $status===FALSE){
            $this->db->insert( 'storage_temperature_converter', $data_post );
    ...
    function update_temperature($id_anon,$company,$data_post){
        $id_anon = trim($this->db->escape_str($id_anon));
        $company = trim($this->db->escape_str($company));
        $status=FALSE;
        if(empty($id_anon)){
            $status = "ID TEMPERATURE BUAH CANNOT BE NULL !!";
        }
        ...
        if(empty($status) || $status==false){
            ...
            $this->db->update('storage_temperature_converter',$data_post);
            if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
    ...
    function delete_temperature($id_anon,$company){
        $id_anon = trim($this->db->escape_str($id_anon));
        $company = trim($this->db->escape_str($company));
        
        $status=FALSE;
        if((!empty($id_anon) && $id_anon==false)){
            $status = "ID_TEMPERATURE CANNOT BE NULL !!";
        }
        ...
        if(empty($status) || $status==false){
            $this->db->where('ID_ANON',$id_anon);
            $this->db->where('COMPANY_CODE',$company);
            $this->db->delete('storage_temperature_converter');
