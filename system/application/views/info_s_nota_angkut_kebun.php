...
{ 
    ...
    $("#search_form").dialog({
        ...
        open:function(){
            jQuery("#list_notaangkut").smartSearchPanel('#search_form', {dialog:{width: 530}},'s_nota_angkut_kebun/search_data');
        }
    });
...
function resetAutocomplete(){
    ...
    $("#txt_noSpb").autocomplete( 
            url+"s_nota_angkut_kebun/get_no_spb/"+$("#txt_tglNota").val()+"/"+cb1, {
...
jQuery(document).ready(function(){
    ...
    $("#txt_noKendaraan").autocomplete( 
            url+"s_nota_angkut_kebun/get_no_kendaraan/", {
              dataType: 'ajax',
              width:350,
              multiple: false,
              limit:20,
...
function reloadGrid(){    
     ...
     jQuery("#list_notaangkut").setGridParam({url:url+'s_nota_angkut_kebun/LoadData/'+get_periode()}).trigger("reloadGrid");    
}
</script>
...
var loadView = function()
        ...
        jGrid_va = jQuery("#list_notaangkut").jqGrid(
            {
                url:url+'s_nota_angkut_kebun/LoadData/'+get_periode(),
                mtype : "POST",
                datatype: "json",
                colNames: colNamesT ,
                colModel: colModelT,
...
,async: false,edittype: "text",editoptions:{
                ...
                $(elem)
                  .autocomplete( 
                    ...
                    url+"s_nota_angkut_kebun/get_afdeling", {
                      dataType: 'ajax',
                      multiple: false,
                      autoFill: false,
                      mustMatch: true,
...
,async: false,edittype: "text",editoptions:{
                ...
                $(elem)
                  .autocomplete(
						...
						url+"s_nota_angkut_kebun/get_block/", {	
                          dataType: 'ajax',
                          width:350,
                          multiple: false,
                          limit:200,
...
var loadView_detail = function()
        ...
        jGrid_va = jQuery("#list_notaDetail").jqGrid(
            {
                url:url+'s_nota_angkut_kebun/LoadData_Detail/',
                mtype : "POST",
                datatype: "json",
			...
			if (data.ID_NT_AB == undefined){
				...
				if (answer){			
					...
					$.ajax({
							type:           'post',
							cache:          false,
							url:            url+'s_nota_angkut_kebun/CRUD_METHOD',
							data:           {myJson:  data} ,
							success: function(msg){
								...
								if(obj.error===true){
									...
									alert(obj.status);									
									//$("#frm_load").reloadGrid(); 							
									jQuery("#list_notaDetail").setGridParam({url:url+"s_nota_angkut_kebun/LoadData_Detail"+"/"+mag}).trigger("reloadGrid");									
									//update_nota();
									$("#frm_load").dialog('close');
...
function add_nota(){
    ...
    jQuery("#list_notaDetail").setGridParam({url:url+"s_nota_angkut_kebun/LoadData_Detail/"}).trigger("reloadGrid"); 
...
function edit_nota(cl){
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
        ...
        jQuery("#list_notaDetail").setGridParam({url:url+"s_nota_angkut_kebun/LoadData_Detail"+"/"+data.ID_NT_AB}).trigger("reloadGrid");  
            ...
            $.ajax({
                    ...
                    success: function(msg){
                        ...
                        if(obj.error===true){
                            ...
                            alert(msg);
                            jQuery("#list_notaDetail").setGridParam({url:url+"s_nota_angkut_kebun/LoadData_Detail/"+
...
function generate_tiket(){
    $.ajax({
        type:           'post',
        cache:          false,
        url:            url+'s_nota_angkut_kebun/generate_tiket',
        success: function(msg){
            $("#txt_noTiket").val(msg);
        }
    });    
...
function generate_spb(){
    $.ajax({
        type:           'post',
        cache:          false,
        url:            url+'s_nota_angkut_kebun/generate_spb',
        success: function(msg){
            $("#txt_noSpb").val(msg);
        }
    });    
...
function gridReload(){ 
    ...
    jQuery("#list_notaangkut").setGridParam({url:url+"s_nota_angkut_kebun/search_spb/"+no_spb+"/"+no_kend+"/"+get_periode()}).trigger("reloadGrid");        
} 
</script>
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
        ...
        if (answer){
            ...
            $.ajax({
                    type:           'post',
                    cache:          false,
                    url:            url+'s_nota_angkut_kebun/CRUD_METHOD/'+'PDF',
                    data:           {myJson:  data} ,
                    success: function(msg){
                        var obj = jQuery.parseJSON(msg);  
                        if(obj.error===true){
