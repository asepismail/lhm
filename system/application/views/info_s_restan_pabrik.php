...
<div id='main_form'>
    <div id"gridSearch">  
        ...
        <table border="0" class="teks_"  cellpadding="234" cellspacing="4">
            <tr><td colspan="8">Pencarian Berdasarkan :</td></tr>
            <tr>
                <td class="labelcell">Periode</td><td class="labelcell">:</td>
                <td class="fieldcell">
                    <input type="text" id='s_tgl_periode' onchange="get_data()" >
                </td>
            </tr>
        </table> -->
    </div>
...
<div id="frm_restan">
    <div id="tabs">
        ...
        <div id="fragment-1" class="panes">
            ...
            <tr>
                <td class="labelcell">Besar Restan (Kg)</td><td>:</td>
                <td class="fieldcell"><input type="text" id='txt_restan' tabindex="2" ></td>
            </tr>
            </table>
...
jQuery(document).ready(function(){
    ...
    $("#search_form").dialog({
        ...
        open:function(){
            jQuery("#list_restan").smartSearchPanel('#search_form', {dialog:{width: 530}},'s_restan_pabrik/search_data');
        }
    });   
})
...
colNamesT.push('ID_RESTAN');
colModelT.push({name:'ID_RESTAN',index:'ID_RESTAN', editable: false, hidden:true, width: 140, align:'left'});
...
colModelT.push({name:'RESTAN',index:'RESTAN', editable: false, hidden:false, width: 140, align:'center'});
...
var loadView = function()
        ...
        jGrid_va = jQuery("#list_restan").jqGrid(
            {
                url:url+'s_restan_pabrik/LoadData/', //+get_periode(),
                mtype : "POST",
                datatype: "json",
                colNames: colNamesT ,
                colModel: colModelT,
...
function reloadGrid(){
     var company_code='<?php echo $company_code;?>'; 
     jQuery("#list_restan").setGridParam({url:url+'s_restan_pabrik/LoadData'}).trigger("reloadGrid");    
}
</script>
...
function delete_restan(cl){
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
        ...
        var answer = confirm ("Hapus Restan dengan ID : " + data.ID_RESTAN + " ?" );
        if (answer){
            $("#frm_load").dialog('open');
            var postdata_id = {};
    
            postdata_id['ID_RESTAN'] = data.ID_RESTAN;
            ...
            $.ajax({
                    type:           'post',
                    cache:          false,
                    url:            url+'s_restan_pabrik/CRUD_METHOD',
                    data:           {myJson:  data} ,
...
function simpan_restan(){
    ...
    if (answer){
        ...
        if(data.ID_RESTAN != null || data.ID_RESTAN != 'undefined' ){
            postdata_id['ID_RESTAN']=data.ID_RESTAN //for update process
        }

        $("#frm_load").dialog('open');
        
        postdata_id['TANGGAL'] = $("#s_tgl_periode_restan").val();
        postdata_id['RESTAN'] =  $("#txt_restan").val();
