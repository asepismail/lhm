...
class m_hpt extends Controller
{
	...
	function get_HPTType(){
        $q = htmlentities($_REQUEST['q'],ENT_QUOTES,'UTF-8'); 
        $data_afd = $this->model_m_hpt->get_HPTType($q);
    ...
    function add_new($data_id){
		...
		$data_post['TGL_DETEKSI']=strtoupper(trim(htmlentities($data_id['TGL_DETEKSI'],ENT_QUOTES,'UTF-8'))) ;
		...
		$TGL_DETEKSI=strval($data_post['TGL_DETEKSI']);
        if(empty($TGL_DETEKSI) || $TGL_DETEKSI==null || $TGL_DETEKSI==false ){
            $return['status']="Tanggal deteksi tidak boleh kosong";
            $return['error']=true;
        }
        
		if(date("Ymd",strtotime($TGL_DETEKSI)) == '19700101' || $TGL_DETEKSI=="UNDEFINED" || $TGL_DETEKSI=="0000-00-00"){
        	$return['status']="format Tanggal deteksi tidak benar";
            $return['error']=true;
        }
