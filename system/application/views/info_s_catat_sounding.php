    ...
    <div id="tabs">
        ...
        <div id="fragment-1" class="panes">
            <table width="100%" border="0" class="teks_" id="input_table" >
                ...
                <tr>
                    <td class="labelcell">VOLUME (Ltr)</td><td>:</td>
                    <td class="fieldcell">
                    	<input tabindex="7" type="text" id="txt_sndVolume" name="snd_input" maxlength="25" disabled="disabled"/>
                    </td>
                </tr>
                <tr>
                    <td class="labelcell">BERAT (Ltr)</td><td>:</td>
                    <td class="fieldcell">
                    	<input tabindex="7" type="text" id="txt_sndWeight" name="snd_input" maxlength="25" />
                    </td>
                </tr>
                 <tr>
                    <td></td><td></td>
                    <td>
                     *Jika Height dibawah meja ukur, input manual field Weight dengan Height = 0 
                    </td>
                </tr>
...
function reloadGrid(){  
     ...
     jQuery("#list_sounding").setGridParam({url:url+'s_catat_sounding/LoadData/'}).trigger("reloadGrid");    
}
...
colModelT.push({name:'ID_SOUNDING',index:'ID_SOUNDING', editable: false, hidden:true, width: 140, align:'left'});
...
,async: false,edittype: "text",editoptions:{
                ...
                $(elem)
                  .autocomplete( 
                    ...
                    url+"s_catat_sounding/get_storage/", {
                      dataType: 'ajax',
                      multiple: false,
                      autoFill: false,
                      mustMatch: true,
...
colNamesT.push('JAM');
colModelT.push({name:'TIME',index:'TIME', editable: true, hidden:false, width: 140, align:'center'});

colNamesT.push('TINGGI (Meter)');    
colModelT.push({name:'HEIGHT',index:'HEIGHT', editable: true, hidden:false, width: 120, align:'left'});

colNamesT.push('TEMPERATURE (C)');
colModelT.push({name:'TEMPERATURE',index:'TEMPERATURE', editable: true, hidden:false, width: 120, align:'left'});

colNamesT.push('VOLUME (Ltr)');
colModelT.push({name:'VOLUME',index:'VOLUME', editable: false, hidden:false, width: 120, align:'left'});

colNamesT.push('BERAT (Kg)');
colModelT.push({name:'WEIGHT',index:'WEIGHT', editable: false, hidden:false, width: 120, align:'left'});
...
var loadView = function()
        ...
        jGrid_va = jQuery("#list_sounding").jqGrid(
            {
                url:url+'s_catat_sounding/LoadData/',
                mtype : "POST",
                datatype: "json",
                colNames: colNamesT ,
                colModel: colModelT,
...
function gridReload(){ 
    ...
    jQuery("#list_sounding").setGridParam({url:url+"s_catat_sounding/search_data/"+kode+"/"+get_periode()}).trigger("reloadGrid");        
}
</script>
...
function simpan_sounding(){
    ...
    if (answer){
        ...
        $.ajax({
                type:           'post',
                cache:          false,
                url:            url+'s_catat_sounding/CRUD_METHOD',
                data:           {myJson:  data} ,
...
function edit_sounding(cl){
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
		...
		var weight = parseInt((data.WEIGHT).replace(",",""));
...
jQuery(document).ready(function(){
   ...
   $("#frm_sounding").dialog({
    ...
    $("#search_form").dialog({
        ...
        open:function(){
            jQuery("#list_sounding").smartSearchPanel('#search_form', {dialog:{width: 530}},'s_catat_sounding/search_data');
        }
    });
});
