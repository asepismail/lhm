...
class model_m_hpt extends Model
{
	...
	*/
	...
	function get_HPTType($company,$location_left,$location){
        $company=$this->db->escape_str($company);
        $location_left=$this->db->escape_str($location_left);
        $location=$this->db->escape_str($location); 
        
        $query="SELECT * FROM s_master_hpt";
        $sQuery=$this->db->query($query);
        $rowcount=$sQuery->num_rows();
    ...
    function add_new($company, $data_post){
        ...
        if( isset($company))
        {
            if(!empty($company))
            {
                $cek_data_exist = $this->cek_data_exist('s_hpt',array('AFD'=>$data_post['AFD'],'BLOK_TANAH'=>$data_post['BLOK_TANAH'], 'BULAN'=>$data_post['BULAN'],'TAHUN'=>$data_post['TAHUN'], 'COMPANY_CODE'=>$company,'ACTIVE'=>1),'ID_HPT'); 
...
,rawat.TIPE_HPT, rawat.PKK_SAMPLE, rawat.PKK_TRS, rawat.TIPE_KATAGORI
FROM (	
