...
<div id='main_form'>
    ...
    <div id="btn_section">
        <button class="testBtn" type="submit" id="add_kontraktor" onclick="input_restan()">Input Curah Hujan</button>&nbsp;
    </div>
...
<div id="frm_restan">
    <div id="tabs">
        ...
        <div id="fragment-1" class="panes">
            ...
            <tr>
                <td class="labelcell">Besar Curah Hujan (mm)</td><td>:</td>
...
jQuery(document).ready(function(){
    ...
    $("#search_form").dialog({
        ...
        open:function(){
            jQuery("#list_restan").smartSearchPanel('#search_form', {dialog:{width: 530}},'s_curah_hujan/search_data');
        }
    });   
})
...
colNamesT.push('ID_CH');
colModelT.push({name:'ID_CH',index:'ID_CH', editable: false, hidden:true, width: 140, align:'left'});
...
colModelT.push({name:'CURAH_HUJAN',index:'CURAH_HUJAN', editable: false, hidden:false, width: 140, align:'center'});
...
var loadView = function()
        ...
        jGrid_va = jQuery("#list_restan").jqGrid(
            {
                url:url+'s_curah_hujan/LoadData/', //+get_periode(),
                mtype : "POST",
                datatype: "json",
                colNames: colNamesT ,
                colModel: colModelT,
...
function reloadGrid(){
     var company_code='<?php echo $company_code;?>'; 
     jQuery("#list_restan").setGridParam({url:url+'s_curah_hujan/LoadData'}).trigger("reloadGrid");    
}
</script>
...
function edit_restan(cl){
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
        ...
        $("#txt_restan").val(data.CURAH_HUJAN);
...
function delete_restan(cl){
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
        ...
        var answer = confirm ("Hapus Curah Hujab dengan ID : " + data.ID_CH + " ?" );
        if (answer){
            $("#frm_load").dialog('open');
            var postdata_id = {};
    
            postdata_id['ID_CH'] = data.ID_CH;
            ...
            $.ajax({
                    type:           'post',
                    cache:          false,
                    url:            url+'s_curah_hujan/CRUD_METHOD',
                    data:           {myJson:  data} ,
...
function simpan_restan(){
    ...
    if (answer){
        ...
        if(data.ID_RESTAN != null || data.ID_RESTAN != 'undefined' ){
            postdata_id['ID_CH']=data.ID_RESTAN //for update process
        }

        $("#frm_load").dialog('open');
        
        postdata_id['TANGGAL'] = $("#s_tgl_periode_restan").val();
        postdata_id['CURAH_HUJAN'] =  $("#txt_restan").val();
