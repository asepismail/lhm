...
class p_progress_bibitan extends Controller 
{
    
	function p_progress_bibitan ()
	{
		parent::Controller();	

		$this->load->model( 'model_p_progress_bibitan' ); 
    ...
    function index()
    {
		$view = "info_p_progress_bibitan";
		$data = array();
		$data['judul_header'] = "Progress Bibitan";
		$data['js'] = "";
	...
	function read_act()
    {
        ...
        echo json_encode($this->model_p_progress_bibitan->read_act($tgl,$company));
    }
	...
	function read_exist_act()
    {
		...
		$company = $this->session->userdata('DCOMPANY');
        echo json_encode($this->model_p_progress_bibitan->read_exist_act($tdate,$lc, $company));
    }
	...
	function cek_pb()
	{
		...
		$lc = $this->uri->segment(4);
		$company = $this->session->userdata('DCOMPANY');
		$data_enroll = $this->model_p_progress_bibitan->cek_pb($tgl, $lc, $company);
		$data = "";
		...
		$prog_date = $this->input->post( 'PROGRESS_TGL_BIBITAN' ); 
				...
				$jumlah = $this->input->post( 'jumlahdt' );
	   				for ($i=1; $i<=$jumlah;$i++) {
						...
						$begID = $company.$tglID;
					$data_post['ID_PROGRESS_BIBITAN'] = $this->global_func->id_GAD('p_progress_bibitan','ID_PROGRESS_BIBITAN',$begID);
						...
						$this->db->from('p_progress_bibitan');
					...
					$insert_id = $this->model_p_progress_bibitan->insert_p_progress_bibitan( $data_post );	
						}  else if ($this->db->count_all_results() != 0) {
							$insert_id = $this->model_p_progress_bibitan->update_p_progress_bibitan( $tgl_progress,$activity,$location,$company, $data_post );
						}
				}			
    	}
	...
	function get_active_act()
	{
		...
		$data_rawat = $this->model_p_progress_bibitan->act($q, $tgl, $company);
	...
	function get_active_loc()
	{
		...
		$data_rawat = $this->model_p_progress_bibitan->blok($q, $act, $tgl, $company);
	...
	function delete()
	{
		...
		$this->model_p_progress_bibitan->delete_data($id,$afd,$tgl,$company);
	}
}
