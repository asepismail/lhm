...
class model_m_rawat_rutin extends Model
{
    ...
    function update_rawat_rutin($data_post, $company){
        ...
        $cek_data_exist = $this->cek_data_exist('s_rawat_rutin',array('ID_RAWAT_RUTIN'=>$data_post['ID_RAWAT_RUTIN']),'ID_RAWAT_RUTIN');
        if ($cek_data_exist <= 0){
        ...
        if(empty($status) || $status==false){
            ...
            $this->db->where('ID_RAWAT_RUTIN',$data_post['ID_RAWAT_RUTIN']);
            $this->db->update('s_rawat_rutin',$data_post);
            if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
