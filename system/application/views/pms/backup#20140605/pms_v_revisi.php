...
colModelT_pengajuan.push({name:'ID',index:'PROJECT_PROP_ID', hidden:true, width: 80, align:'center'});
	...
	function submit_detailppj(inisial){
		...
		$.post( url+"pms_c_pengajuan/insert_detail/", postdata, function(message) {
				if(message.replace(/\s/g,"") != 0 ) { 
					...
					jQuery("#list_pengajuan").setGridParam({url:url+'pms_c_revisi/read_revproject/'+$.trim($("#i_ino_pengajuan").val())}).trigger("reloadGrid");
					if(inisial !== "simpan"){
...
/* cari blok */
    ...
    function gridProjectReload(){ 
       var q = jQuery("#iSproject").val();
       jQuery("#list_sProject").setGridParam({url:url+"pms_c_revisi/listProject/"+q}).trigger("reloadGrid");        
    } 
	...
	/* ######### cari project ################## */
 	var grid_sProject = null;
    var colNames_sProject = new Array();
    var colModel_sProject = new Array(); 
    
    colNames_sProject.push('id');
    colModel_sProject.push({name:'ID',index:'ID', editable: true,hidden:true, width: 30, align:'center'});

    colNames_sProject.push('Kode Project');
    colModel_sProject.push({name:'PROJECT_ID',index:'PROJECT_ID', editable: true,hidden:false, width: 40, align:'center'});
            
    colNames_sProject.push('Afd');
    colModel_sProject.push({name:'AFD',index:'AFD', editable: true,hidden:false, width: 20, align:'center'});
            
    colNames_sProject.push('Tipe');
    colModel_sProject.push({name:'PROJECT_TYPE',index:'PROJECT_TYPE', editable: true,hidden:false, width: 20, align:'center'});

    colNames_sProject.push('Subtipe');
    colModel_sProject.push({name:'PROJECT_SUBTYPE',index:'PROJECT_SUBTYPE',  editable: true,hidden:true, width:30, align:'left'});
	
	colNames_sProject.push('Deskripsi');
    colModel_sProject.push({name:'PROJECT_DESC',index:'PROJECT_DESC', editable: true,hidden:false, width:120, align:'left'});
	
	colNames_sProject.push('Lokasi');
    colModel_sProject.push({name:'PROJECT_LOCATION',index:'PROJECT_LOCATION', editable: true,hidden:false, 
						   width:60, align:'center'});

   	colNames_sProject.push('Aktivitas');
    colModel_sProject.push({name:'PROJECT_ACTIVITY',index:'PROJECT_ACTIVITY', editable: true,hidden:false, 
						   width: 35, align:'center'});
	
	colNames_sProject.push('Aktif');
    colModel_sProject.push({name:'PROJECT_STATUS',index:'PROJECT_STATUS', 
	...
	colNames_sProject.push('Tgl Terbit');
    colModel_sProject.push({name:'TGL_TERBIT',index:'TGL_TERBIT', editable: true,hidden:false, width: 40, align:'center'});
	
	colNames_sProject.push('COMPANY_CODE');
    colModel_sProject.push({name:'COMPANY_CODE',index:'COMPANY_CODE', editable: true,hidden:true, width: 50, align:'center'});
	
	var loadView_sProject = function(){
        jgrid_sProject = jQuery("#list_sProject").jqGrid({
            url:url+'pms_c_revisi/listProject/',  
            datatype: 'json',  mtype: 'POST', colNames:colNames_sProject, colModel:colModel_sProject,
            pager: jQuery('#pager_sProject'),  rownumbers: true, rowNum: 20, width:650, 
            height: 200, sortorder: "asc", forceFit : true, rowList:[10,20,30], 
            multiple:false, ondblClickRow: function(){
				var id = jQuery("#list_sProject").getGridParam('selrow');
				if (id)	{
					var ret = jQuery("#list_sProject").getRowData(id);
					jQuery("#sProject").val(ret.PROJECT_ID)
					$("#sFormProject").dialog("close");
					showMainForm();
				}
			}, caption: 'Daftar Project', editurl:url+'pms_c_revisi/listProject/',
			loadComplete: function(){}, sortname: colModel_sProject[1].name,  sortorder: "desc",  viewrecords: true    
        });
        jgrid_sProject.navGrid('#pager_sProject',{edit:false,del:false,add:false, search: false, refresh: true});
        $("#alertmod").remove();//FIXME         
    }
    jQuery("#list_sProject").ready(loadView_sProject);
...
<span>
  Kode Project : <input tabindex="2" type="text" style="width:100px;" class="input" disabled="disable" id="sProject" />
  <div id="searchProject" style=" position:relative; margin-left:205px; margin-top:-16px;">
    <img id="searchButton" src="<?= $template_path ?>themes/base/images/Search.png" style="cursor:pointer;" onclick="OpenSearchProject()" />&nbsp;&nbsp;
...
<tr>
  <td><table width="95%" >
    ...
    <tr>
      ...
      <td>:</td>
      <td><input tabindex="3" type="text" style="width:120px;" class="input" id="i_tgl_ppj" /><br/><br/></td>
    </tr>
    <!-- <tr>
...
<table width="100%" border="1">
	<tr>
    	<td colspan="3" valign="middle">
    		<span> Cari Project : </span> <input type="text" id="iSproject" class="input" onkeydown="doSearchProject(arguments[0]||event)" />
        </td>
  	</tr>
	<tr>
    	<td colspan="3">
        	<br />
    		<table id="list_sProject" class="scroll" cellpadding="0" cellspacing="0"></table>
   			<div id="pager_sProject" class="scroll"></div>
    	</td>
  	</tr>
</table>
</div>