...
class p_progress_rawat extends Controller 
{

	function p_progress_rawat ()
	{
		parent::Controller();	
		/*modul yang di load halaman vehicle activity*/
		$this->load->model( 'model_progress_rawat' ); 
	...
	function index()
    {
		$view = "info_p_progress_rawat";
		$data = array();
		$data['judul_header'] = "Progress Rawat";
		$data['js'] = "";
	...
	function dropdownlist_afd()
	{
		...
		$data_afd = $this->model_progress_rawat->get_afdeling($this->session->userdata('DCOMPANY'));
	...
	function dropdownlist_block()
	{
		...
		$data_afd = $this->model_progress_rawat->get_block($afd,$this->session->userdata('DCOMPANY'));
	...
	function read_act()
    {
        ...
        echo json_encode($this->model_progress_rawat->read_act($tgl,$lc, $company));
    }
	...
	function read_exist_act()
    {
		...
		$lc = $this->uri->segment(4);
		$company = $this->session->userdata('DCOMPANY');
        echo json_encode($this->model_progress_rawat->read_exist_act($tdate,$lc, $company));
    }
	...
	function cek_pr(){
		...
		$lc = $this->uri->segment(4);
		$company = $this->session->userdata('DCOMPANY');
		$data_enroll = $this->model_progress_rawat->cek_pr($tgl, $lc, $company);
  			...
  			//$this->db->get('m_gang_activity_detail');
					...
					$jumlah = $this->input->post( 'jumlahdt' );
	   				for ($i=1; $i<=$jumlah;$i++) {
						...
						$data_post['ID_PROGRESS_RAWAT'] = $this->global_func->id_GAD('p_progress_rawat','ID_PROGRESS_RAWAT',$begID);
						...
						if ($this->db->count_all_results() == 0) {
							$insert_id = $this->model_progress_rawat->insert_progress_rawat( $data_post );						
						} else if ($this->db->count_all_results() != 0) {
							$insert_id = $this->model_progress_rawat->update_progress_rawat( $afd,$tgl_progress,$activity,$location,$company, $data_post );	
	...
	function get_active_act()
	{
		...
		$data_rawat = $this->model_progress_rawat->blok($afd, $q, $tgl, $company);
	...
	function get_active_loc()
	{
		...
		$data_rawat = $this->model_progress_rawat->act($afd, $q, $act, $tgl, $company);
	...
	function delete()
	{
		...
		$this->model_progress_rawat->delete_data($id,$afd,$tgl,$company);
	}
