...
class model_s_nota_angkut extends Model{
    ...
    function LoadData($company,$periode){
		...
		if ($flag_wp == "0"){
			...
			$queries = "SELECT s_nota_angkutbuah.*, s_data_timbangan.BERAT_BERSIH, s_data_timbangan.FLAG_TIMBANGAN FROM s_nota_angkutbuah INNER JOIN s_data_timbangan ON s_nota_angkutbuah.NO_SPB = s_data_timbangan.NO_SPB WHERE s_nota_angkutbuah.ACTIVE=1 AND s_nota_angkutbuah.COMPANY_CODE='".$company."' AND DATE_FORMAT(s_nota_angkutbuah.TANGGAL,'%Y%m')='".$periode."'"; 
		}
