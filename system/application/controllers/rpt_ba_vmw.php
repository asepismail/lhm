<?
class rpt_ba_vmw extends Controller 
{
    function rpt_ba_vmw ()
    {
        parent::Controller();    
...
// ------------------------------ vehicle mesin workshop punya ------------------------------------------------- //
    function js_ba_vmw(){
        ...
        jQuery('#submitdata').click(function (){
            ...
            if ( jns_laporan == 'html'){
                urls = url + 'rpt_ba_vmw/ba_vmw_rekap_afd/' + $('#FROM').val() + '/' + $('#TO').val();
                $('#frame').attr('src',urls); 
            } else if ( jns_laporan == 'excell'){
                urls = url + 'rpt_ba_vmw/ba_xlsvmw_rekap/' + $('#FROM').val() + '/' + $('#TO').val(); 
                $.download(urls,'');
            }
        });";
        return $js;
    ...
    function ba_vmw_rekap_afd($from,$to){
        ...
        $data_vmw = $this->model_rpt_ba->ba_vmw_afd($from, $to, $periode,$company);
        $total = 0;
        $total_hk = 0;
...
</tr><tr><td colspan='3' align='center'><strong>NO : &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; / TEK / ".$company." / ".$bulanr." / ".$tahun." </strong></td>
 ...
 $tabel .= "<tr><th class='tbl_th'>BLN INI</th><th class='tbl_th'>s.d BLN INI</th></tr>";
    ...
    function ba_xlsvmw_rekap($from,$to){
       	$from = str_replace("-","",$this->uri->segment(3));
        $to = str_replace("-","",$this->uri->segment(4));
        $periode = substr(str_replace("-","",$to),0,6);
        $company = $this->session->userdata('DCOMPANY');
       ...
       $bulan = $this->bln_to_periode($bulan);
        ...
        $judul .= "NO :     / TEK / ".$company." / ".$bulanr." / ".$tahun."\n";
        ...
        header("Content-Disposition: attachment; filename=BA_TEKNIK_".$company."_".$periode.".xls");
        echo "$judul\n$headers\n$data";  
    }
