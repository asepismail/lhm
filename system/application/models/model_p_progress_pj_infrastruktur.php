...
class model_p_progress_pj_infrastruktur extends Model 
{

    function model_p_progress_pj_infrastruktur()
    {
        parent::Model(); 
	...
	function insert_p_progress_pj_infrastruktur ( $data )
	{
	...
	function update_p_progress_pj_infrastruktur ( $tgl_progress,$activity,$location,$company, $data )
	{
	...
	function read_act($tgl,$lc, $company)
    {
		...
		if($lc != '' || $lc != 'xx'){
			$where = "pj.PROJECT_LOCATION LIKE '".$lc."%'";
		} else {
			$where = "1 = 1";
		}
		
		$sql2 = "SELECT lhm.LHM_DATE, lhm.LOCATION_CODE, lhm.ACTIVITY_CODE,pj.PROJECT_LOCATION AS PJ_LOCATION, pj.AFD AS AFD, pm.ACCOUNTDESC AS DESCR, pm.UNIT1, pm.UNIT2 
				...
				AND lhm.ACTIVITY_CODE IN (SELECT ACCOUNTCODE FROM m_progress_map WHERE PENGGUNAAN = 'IF' AND parent = 0) 
		...
		$sql = "SELECT lhm.LHM_DATE, lhm.LOCATION_CODE, lhm.ACTIVITY_CODE, pj.PROJECT_LOCATION AS PJ_LOCATION, pj.AFD AS AFD,pm.ACCOUNTDESC AS DESCR, 
				pm.UNIT1, pm.UNIT2 FROM m_gang_activity_detail lhm 
				LEFT JOIN m_progress_map pm ON (pm.ACCOUNTCODE = lhm.ACTIVITY_CODE) 
				LEFT JOIN m_project pj ON pj.PROJECT_ID = lhm.LOCATION_CODE 
				WHERE ".$where." AND lhm.COMPANY_CODE = '".$company."' AND DATE_FORMAT(lhm.LHM_DATE,'%Y%m%d') = '".$tgl."' 
				AND lhm.ACTIVITY_CODE IN (SELECT ACCOUNTCODE FROM m_progress_map WHERE PENGGUNAAN = 'IF' 
				AND parent = 0) GROUP BY lhm.LHM_DATE, lhm.LOCATION_CODE, lhm.ACTIVITY_CODE ORDER BY ".$sidx." ".$sord.""; 

		$objects = $this->db->query($sql,FALSE)->result(); 
        $rows =  array();
		$ID_PROGRESS_INFRASTRUKTUR = 1;
		$HASIL_KERJA = "";
		foreach($objects as $obj)
        {
            $cell = array();
				...
				array_push($cell, $ID_PROGRESS_INFRASTRUKTUR);
				...
				array_push($cell, $obj->PJ_LOCATION);
	...
	function cek_if($tgl, $lc, $company){
		$query = $this->db->query("select count(*) as jumlah FROM p_progress_infrastruktur where PJ_LOCATION LIKE '".$lc."%' and DATE_FORMAT(TGL_PROGRESS, '%Y%m%d') = '".$tgl."' and COMPANY_CODE = '".$company."'");
		$temp_result = array();
		foreach ( $query->result_array() as $row )
		{
			$temp_result [] = $row;	
	...
	function read_exist_act($tgl,$lc, $company)
    {
		...
		if($lc != ''){
			$where = "PJ_LOCATION LIKE '".$lc."%'";
		} else {
			$where = "1 = 1";
		}		
		
		$sql2 = "SELECT ID_PROGRESS_INFRASTRUKTUR, TGL_PROGRESS, LOCATION_CODE, PJ_LOCATION, AFD, ACTIVITY_CODE, ACTIVITY_DESC, SATUAN, 
	HASIL_KERJA, COMPANY_CODE FROM p_progress_infrastruktur
WHERE ".$where." AND DATE_FORMAT(TGL_PROGRESS, '%Y%m%d') = '".$tgl."' AND COMPANY_CODE = '".$company."'";
		...
		$sql = "SELECT ID_PROGRESS_INFRASTRUKTUR, TGL_PROGRESS, LOCATION_CODE, PJ_LOCATION, AFD, ACTIVITY_CODE, ACTIVITY_DESC, SATUAN, 
	HASIL_KERJA, COMPANY_CODE FROM p_progress_infrastruktur
WHERE ".$where." AND DATE_FORMAT(TGL_PROGRESS, '%Y%m%d') = '".$tgl."' AND COMPANY_CODE = '".$company."' 
ORDER BY ".$sidx." ".$sord.""; 
		...
		foreach($objects as $obj)
        {
            $cell = array();
				...
				array_push($cell, $obj->ID_PROGRESS_INFRASTRUKTUR);
	...
	function act($loc, $q, $tgl, $company){
		...
		$query = $this->db->query("SELECT LOCATION_CODE, ACTIVITY_CODE, m_coa.COA_DESCRIPTION AS DESCR,  m_progress_map.UNIT1 AS UNIT1
			...
			AND m_project.PROJECT_LOCATION LIKE '".$loc."%' 
			AND m_gang_activity_detail.COMPANY_CODE = '".$company."'
			AND m_gang_activity_detail.ACTIVITY_CODE IN (SELECT ACCOUNTCODE FROM m_progress_map WHERE PENGGUNAAN = 'IF') 
	...
	function blok($q, $act, $tgl, $company){
		...
		$query = $this->db->query("SELECT LOCATION_CODE, m_project.PROJECT_LOCATION as PJ_LOCATION,m_project.AFD AS AFD FROM m_gang_activity_detail 
				LEFT JOIN m_project ON m_project.PROJECT_ID =  m_gang_activity_detail.LOCATION_CODE
				WHERE DATE_FORMAT(LHM_DATE,'%Y%m%d') = '".$tgl."'
	...
	{
		$this->db->where( 'ID_PROGRESS_INFRASTRUKTUR', $id ); 
		$this->db->where( 'LOCATION_CODE', $afd ); 
		$this->db->where( 'TGL_PROGRESS', $tgl ); 
		$this->db->where( 'COMPANY_CODE', $company ); 	
		$this->db->delete('p_progress_infrastruktur');   
	}
}