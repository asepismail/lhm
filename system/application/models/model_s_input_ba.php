...
WHERE a.COMPANY_CODE = '".$company."' AND a.BA_DATE = ('".$dates."' + INTERVAL 1 DAY) AND a.ACTIVE=1";
	...
	function get_tbs($company, $date){
        ...
        $company=htmlentities($this->db->escape_str($company),ENT_QUOTES,'UTF-8');
		if($company=='MAG' || $company=='LIH'){
			/*
			$sql="SELECT DISTINCT(timbangan.TANGGALM) AS TANGGAL, timbangan.COMPANY_CODE, timbangan.JENIS_MUATAN, 
	coalesce(tbs_inti.TBS_INTI,0) AS TBS_INTI, coalesce(tbs_supplier.TBS_SUPPLIER,0) AS TBS_SUPPLIER, coalesce(tbs_group.TBS_GROUP,0) AS TBS_GROUP, coalesce(tbs_plasma.TBS_PLASMA,0) AS TBS_PLASMA					    
	FROM s_data_timbangan timbangan		
	LEFT JOIN (
		SELECT T.TANGGALM AS TANGGAL, COALESCE(SUM(T.BERAT_BERSIH),0) AS TBS_INTI				    
		FROM s_data_timbangan T			
		WHERE T.COMPANY_CODE = '".$company."'
		AND T.TANGGALM = '".$date."'
		AND T.JENIS_MUATAN= 'TBS'
		AND T.TYPE_BUAH = 1
		GROUP BY T.TYPE_BUAH
	) tbs_inti ON timbangan.TANGGALM = tbs_inti.TANGGAL
	LEFT JOIN (
		SELECT s.TANGGALM AS TANGGAL, -- COALESCE(SUM(s.BERAT_BERSIH),0) AS TBS_SUPPLIER,
		COALESCE(SUM((s.BERAT_ISI-s.BERAT_KOSONG)*(s.GRD_LAINNYA/100))+SUM(s.BERAT_BERSIH),0) AS TBS_SUPPLIER		    
		FROM s_data_timbangan s			
		WHERE s.COMPANY_CODE='".$company."'
		AND s.TANGGALM = '".$date."'
		AND s.JENIS_MUATAN= 'TBS'
		AND s.TYPE_BUAH = 2
		GROUP BY s.TYPE_BUAH
	) tbs_supplier ON timbangan.TANGGALM = tbs_supplier.TANGGAL
	LEFT JOIN (
		SELECT g.TANGGALM AS TANGGAL, COALESCE(SUM(g.BERAT_BERSIH),0) AS TBS_GROUP			    
		FROM s_data_timbangan g			
		WHERE g.COMPANY_CODE = '".$company."'
		AND g.TANGGALM = '".$date."'
		AND g.JENIS_MUATAN= 'TBS'
		AND g.TYPE_BUAH = 4
		GROUP BY g.TYPE_BUAH
	) tbs_group ON timbangan.TANGGALM = tbs_group.TANGGAL
	LEFT JOIN (
		SELECT p.TANGGALM AS TANGGAL, COALESCE(SUM(p.BERAT_BERSIH),0) AS TBS_PLASMA			    
		FROM s_data_timbangan p			
		WHERE p.COMPANY_CODE = '".$company."'
		AND p.TANGGALM = '".$date."'
		AND p.JENIS_MUATAN= 'TBS'
		AND p.TYPE_BUAH = 3
		GROUP BY p.TYPE_BUAH
	) tbs_plasma ON timbangan.TANGGALM = tbs_plasma.TANGGAL
	WHERE timbangan.COMPANY_CODE='".$company."' 
	AND timbangan.TANGGALM = '".$date."'
	AND timbangan.JENIS_MUATAN= 'TBS'";
	*/
			$sql="SELECT DISTINCT(timbangan.TANGGALK) AS TANGGAL, timbangan.COMPANY_CODE, timbangan.JENIS_MUATAN, 
	coalesce(tbs_inti.TBS_INTI,0) AS TBS_INTI, coalesce(tbs_supplier.TBS_SUPPLIER,0) AS TBS_SUPPLIER, coalesce(tbs_group.TBS_GROUP,0) AS TBS_GROUP, coalesce(tbs_plasma.TBS_PLASMA,0) AS TBS_PLASMA					    
	FROM s_data_timbangan timbangan		
	LEFT JOIN (
		SELECT DATE_FORMAT(INTI.TANGGAL, '%Y-%m-%d') AS TANGGAL, COALESCE(SUM(INTI.BERAT_BERSIH),0) AS TBS_INTI
		FROM(
		...
		WHERE INTI.TANGGAL BETWEEN '".$date." 07:00:00' AND '".$next_day." 06:59:59'
	) tbs_inti ON timbangan.TANGGALK = tbs_inti.TANGGAL
	LEFT JOIN (
		SELECT DATE_FORMAT(SUPPLIER.TANGGAL, '%Y-%m-%d') AS TANGGAL
		,COALESCE(SUM((SUPPLIER.BERAT_ISI-SUPPLIER.BERAT_KOSONG)*(SUPPLIER.GRD_LAINNYA/100))+SUM(SUPPLIER.BERAT_BERSIH),0) AS TBS_SUPPLIER	
		FROM(
			SELECT CAST(STR_TO_DATE(CONCAT(TANGGALK, WAKTUK), '%Y-%m-%d %H:%i:%s') AS DATETIME) AS TANGGAL, 
			BERAT_ISI, BERAT_KOSONG, BERAT_BERSIH, GRD_LAINNYA		    
			FROM s_data_timbangan 			
			WHERE COMPANY_CODE = '".$company."'
			AND TANGGALK between '".$date."' AND '".$next_day."'
			AND JENIS_MUATAN= 'TBS'
		...
		WHERE SUPPLIER.TANGGAL BETWEEN '".$date." 07:00:00' AND '".$next_day." 06:59:59'		
	) tbs_supplier ON timbangan.TANGGALK = tbs_supplier.TANGGAL
	LEFT JOIN (	
		SELECT DATE_FORMAT(GROUPS.TANGGAL, '%Y-%m-%d') AS TANGGAL, COALESCE(SUM(GROUPS.BERAT_BERSIH),0) AS TBS_GROUP
		FROM(
			SELECT CAST(STR_TO_DATE(CONCAT(TANGGALK, WAKTUK), '%Y-%m-%d %H:%i:%s') AS DATETIME) AS TANGGAL, BERAT_BERSIH		    
			FROM s_data_timbangan 			
			WHERE COMPANY_CODE = '".$company."'
		...
		WHERE GROUPS.TANGGAL BETWEEN '".$date." 07:00:00' AND '".$next_day." 06:59:59'
	) tbs_group ON timbangan.TANGGALK = tbs_group.TANGGAL
	LEFT JOIN (		
		SELECT DATE_FORMAT(PLASMA.TANGGAL, '%Y-%m-%d') AS TANGGAL, COALESCE(SUM(PLASMA.BERAT_BERSIH),0) AS TBS_PLASMA
		FROM(
			SELECT CAST(STR_TO_DATE(CONCAT(TANGGALK, WAKTUK), '%Y-%m-%d %H:%i:%s') AS DATETIME) AS TANGGAL, BERAT_BERSIH		    
			FROM s_data_timbangan 			
			WHERE COMPANY_CODE = '".$company."'
		...
		WHERE PLASMA.TANGGAL BETWEEN '".$date." 07:00:00' AND '".$next_day." 06:59:59'
	) tbs_plasma ON timbangan.TANGGALK = tbs_plasma.TANGGAL
	WHERE timbangan.COMPANY_CODE='".$company."' 
	AND timbangan.TANGGALK = '".$date."'
	AND timbangan.JENIS_MUATAN= 'TBS'";
		}else if($company=='SMI'){
			$sql="SELECT DISTINCT(timbangan.TANGGALK) AS TANGGAL, timbangan.COMPANY_CODE, timbangan.JENIS_MUATAN, 
	coalesce(tbs_inti.TBS_INTI,0) AS TBS_INTI, coalesce(tbs_supplier.TBS_SUPPLIER,0) AS TBS_SUPPLIER, coalesce(tbs_group.TBS_GROUP,0) AS TBS_GROUP, coalesce(tbs_plasma.TBS_PLASMA,0) AS TBS_PLASMA					    
	...
	LEFT JOIN (
		SELECT DATE_FORMAT(SUPPLIER.TANGGAL, '%Y-%m-%d') AS TANGGAL
		,COALESCE(SUM(SUPPLIER.BERAT_ISI-SUPPLIER.BERAT_KOSONG),0) AS TBS_SUPPLIER	
		FROM(
			SELECT CAST(STR_TO_DATE(CONCAT(TANGGALK, WAKTUK), '%Y-%m-%d %H:%i:%s') AS DATETIME) AS TANGGAL, 
			BERAT_ISI, BERAT_KOSONG, BERAT_BERSIH, GRD_LAINNYA		    
			FROM s_data_timbangan 			
	...
	LEFT JOIN (	
		...
		FROM(
			SELECT CAST(STR_TO_DATE(CONCAT(TANGGALK, WAKTUK), '%Y-%m-%d %H:%i:%s') AS DATETIME) AS TANGGAL, BERAT_BERSIH		    
			FROM s_data_timbangan 			
			WHERE COMPANY_CODE IN ('ASL','SML')
			AND TANGGALK between '".$date."' AND '".$next_day."'
			AND JENIS_MUATAN= 'TBS'
			AND TYPE_BUAH = 4
			AND NO_TIKET NOT LIKE 'GKM%'
			ORDER BY TANGGALK
		)GROUPS
		WHERE GROUPS.TANGGAL BETWEEN '".$date." 07:00:00' AND '".$next_day." 06:59:59'
	) tbs_group ON timbangan.TANGGALK = tbs_group.TANGGAL
	...
	LEFT JOIN (
		SELECT s.TANGGALM AS TANGGAL,
		COALESCE(SUM(s.BERAT_BERSIH),0) AS TBS_SUPPLIER	    
		FROM s_data_timbangan s			
		WHERE s.COMPANY_CODE='".$company."'
		AND s.TANGGALM = '".$date."'
		AND s.JENIS_MUATAN= 'TBS'
	...
	LEFT JOIN (
		SELECT g.TANGGALM AS TANGGAL, COALESCE(SUM(g.BERAT_BERSIH),0) AS TBS_GROUP			    
		FROM s_data_timbangan g			
		WHERE g.COMPANY_CODE IN ('ASL','SSS','SML')
		AND g.TANGGALM = '".$date."'
		AND g.JENIS_MUATAN= 'TBS'
		AND g.TYPE_BUAH = 4
		GROUP BY g.TYPE_BUAH
	...
	LEFT JOIN (
		SELECT DATE_FORMAT(SUPPLIER.TANGGAL, '%Y-%m-%d') AS TANGGAL, COALESCE(SUM(SUPPLIER.BERAT_ISI-SUPPLIER.BERAT_KOSONG),0) AS TBS_SUPPLIER
		FROM(
			SELECT CAST(STR_TO_DATE(CONCAT(TANGGALK, WAKTUK), '%Y-%m-%d %H:%i:%s') AS DATETIME) AS TANGGAL, BERAT_ISI, BERAT_KOSONG, BERAT_BERSIH		    
			FROM s_data_timbangan 			
			WHERE COMPANY_CODE = '".$company."'
			AND TANGGALK between '".$date."' AND '".$next_day."'
			AND JENIS_MUATAN= 'TBS'
	...
	LEFT JOIN (
		SELECT DATE_FORMAT(GROUPS.TANGGAL, '%Y-%m-%d') AS TANGGAL, COALESCE(SUM(GROUPS.BERAT_ISI-GROUPS.BERAT_KOSONG),0) AS TBS_GROUP
		FROM(
			SELECT CAST(STR_TO_DATE(CONCAT(TANGGALK, WAKTUK), '%Y-%m-%d %H:%i:%s') AS DATETIME) AS TANGGAL, BERAT_ISI, BERAT_KOSONG, BERAT_BERSIH		    
			FROM s_data_timbangan 			
			WHERE COMPANY_CODE IN ('ASL','SSS','SML')
			AND TANGGALK between '".$date."' AND '".$next_day."'
			AND JENIS_MUATAN= 'TBS'
			AND TYPE_BUAH = 4
			AND NO_TIKET LIKE 'GKM%'
			ORDER BY TANGGALK
		)GROUPS
		WHERE GROUPS.TANGGAL BETWEEN '".$date." 07:00:00' AND '".$next_day." 06:59:59'
	) tbs_group ON timbangan.TANGGALK = tbs_group.TANGGAL
	...
	WHERE timbangan.COMPANY_CODE IN ('SSS','SML','GKM','ASL') 
	AND timbangan.TANGGALK = '".$date."'
	AND timbangan.JENIS_MUATAN= 'TBS'";
	...
	function get_restan($company, $date){
        $company=htmlentities($this->db->escape_str($company),ENT_QUOTES,'UTF-8');
        $sql="SELECT R.BALANCE
			FROM s_ba R
			WHERE R.ACTIVE =1 AND R.COMPANY_CODE='".$company."' 
			AND R.BA_DATE = ('".$date."' - INTERVAL 1 DAY)
			;";
        $query = $this->db->query($sql);
		if($query->num_rows() > 0){
            $row = $query->row();            
	...
	function get_grading($company, $date, $type){
        $company=htmlentities($this->db->escape_str($company),ENT_QUOTES,'UTF-8');
        $sql="SELECT AVG(".$type.")	AS GRADING		    
FROM s_data_timbangan		
WHERE COMPANY_CODE='".$company."' 
AND TANGGALM = '".$date."' 
AND JENIS_MUATAN= 'TBS'
...
AND k.KODE_JENIS IN ('CPO', 'KRN', 'TNK', 'CKG', 'ABJ')";
        ...
        foreach($objects as $obj){
			...
			$weight_today =0;
			if ($obj->JENIS == 'CPO'){
				$weight=$this->get_prod_cpo($obj->COMPANY_CODE, $obj->JENIS, $date);
			}else if ($obj->JENIS == 'KERNEL'){
				$weight=$this->get_prod_kernel($obj->COMPANY_CODE, $obj->JENIS, $date);
...
WHERE k.ACTIVE=1
AND k.COMPANY_CODE ='".$company."' 
AND k.PRODUCT_CODE IN ('CPO', 'KERNEL')";
	...
	function LoadDispatchByDate($company,$date){
        ...
        $company=trim($this->db->escape_str($company)); 
		$queries = "SELECT k.ID_KOMODITAS AS ID_COMMODITY , k.JENIS AS JENIS, k.COMPANY_CODE,'' AS ID_DISPATCH, '' AS FFA, '' AS MOISTURE, '' AS DIRT
FROM s_komoditas k
WHERE k.ACTIVE=1
AND k.COMPANY_CODE ='".$company."'  
AND k.KODE_JENIS IN ('CPO', 'KRN', 'TNK', 'CKG', 'ABJ')";
       ...
       //var_dump($sql2);
        ...
        foreach($objects as $obj){
			$weight = 0;
			if ($obj->JENIS=='TANKOS'){
				$weight=$this->get_dispatch_prod($obj->COMPANY_CODE, $obj->JENIS, $date);	
			}else{
				$weight=$this->get_dispatch($obj->COMPANY_CODE, $obj->JENIS, $date);
			}
			$weight = floor($weight);
			$weight = $this->rounds($weight);
            $cell = array();
	...
	function LoadOtherStockByDate($company,$date){
		...
		$queries="SELECT k.ID_KOMODITAS AS ID_COMMODITY, k.JENIS AS COMMODITY, k.COMPANY_CODE, '' AS ID_PRODUCTION FROM s_komoditas k
					WHERE k.ACTIVE=1
					AND k.COMPANY_CODE ='".$company."' 
        ...
        $no = 1; 
        foreach($objects as $obj){			
			$weight=$this->get_otherstock($obj->COMPANY_CODE, $obj->ID_COMMODITY, $date);
			$weight = floor($weight);
			$weight = $this->rounds($weight);
            $cell = array();
            array_push($cell, $no);
	...
	function get_dispatch($company, $jenis, $date){
		...
		if (($company=='GKM' || $company=='LIH') && ($jenis=='CANGKANG' || $jenis=='CKG')){
			/*
			$query="SELECT COALESCE(SUM(BERAT_BERSIH),0) AS VOL_DISPATCH
			FROM s_dispatch
		...
		}else if (($company=='GKM' || $company=='LIH') && ($jenis=='KERNEL' || $jenis=='KRN')){
			/*
			$query="SELECT COALESCE(SUM(BERAT_BERSIH),0) AS VOL_DISPATCH
			FROM s_dispatch
			WHERE s_dispatch.ACTIVE='1' 
...
//var_dump($query);
	...
	function get_otherstock($company, $jenis, $date){
        $company = $this->db->escape_str($company);
		
        $query="SELECT COALESCE(SUM(WEIGHT),0) AS VOL_DISPATCH
		FROM s_production
		WHERE ACTIVE='1' 
		AND COMPANY_CODE='".$company."' 
		AND ID_COMMODITY='".$jenis."' 
	...
	$query="SELECT  TANGGALH AS TANGGAL_PRODUKSI, COALESCE(DISPATCH.VOL_DISPATCH,0) AS DISPATCH, COALESCE(SOUNDINGH.WEIGHTH,0) AS WEIGHTH,
			 SUM(CASE WHEN COALESCE(SOUNDINGK.WEIGHTK,0) = 0  THEN (
				...
				AND DATE_FORMAT(snd2.DATE,'%Y%m%d') = DATE_FORMAT((SELECT MAX(DATE)
																	...
																	WHERE snd1.DATE < DATE_FORMAT(TANGGALH,'%Y%m%d') AND snd1.COMPANY_CODE='".$company."'
																	AND snd1.ACTIVE=1),'%Y%m%d') 					
			)ELSE SOUNDINGK.WEIGHTK END) AS WEIGHTK, coalesce(SOUNDINGH.ADJUSTH,0) AS ADJUSTH, COMPANY_CODE
	FROM
	(
		SELECT SUM(snd.WEIGHT) AS WEIGHTH, SUM(ADJUST.SLUDGE) AS ADJUSTH, snd.COMPANY_CODE, snd.`DATE` AS TANGGALH
		FROM s_sounding snd 			
        ...
        $query="	SELECT  TANGGALH AS TANGGAL_PRODUKSI, COALESCE(DISPATCHKERNEL.VOL_DISPATCH,0) AS DISPATCH, COALESCE(SOUNDINGKERNELH.WEIGHTH,0) AS WEIGHTH,
			 SUM(CASE WHEN COALESCE(SOUNDINGKERNELK.WEIGHTK,0) = 0  THEN (
				...
				AND DATE_FORMAT(snd2.DATE,'%Y%m%d') = DATE_FORMAT((SELECT MAX(DATE)
																	...
																	WHERE snd1.DATE < DATE_FORMAT(TANGGALH,'%Y%m%d') AND snd1.COMPANY_CODE='".$company."'
																	AND snd1.ACTIVE=1),'%Y%m%d') 					
			)ELSE SOUNDINGKERNELK.WEIGHTK END) AS WEIGHTK, COMPANY_CODE
	FROM
	(
		SELECT snd.ID_STORAGE, SUM(snd.WEIGHT) AS WEIGHTH ,snd.COMPANY_CODE, snd.DATE AS TANGGALH
...
WHERE s.ACTIVE= 1 AND s.COMPANY_CODE = '".$company."'";
	...
	function get_ffb_min($company_code, $dates){
		$query="SELECT * FROM s_ba ba
				WHERE 
				ba.ACTIVE = 1
				AND ba.COMPANY_CODE = '".$company_code."'
				AND ba.BA_DATE = (
					SELECT MIN(ba.BA_DATE) MIN_DATE FROM s_ba ba
					WHERE ba.COMPANY_CODE = '".$company_code."'
					AND DATE_FORMAT(ba.BA_DATE,'%Y%m') = DATE_FORMAT('".$dates."','%Y%m')
					AND ba.ACTIVE = 1
				)";
...
k.KODE_JENIS FROM s_ba_production p
INNER JOIN s_komoditas k ON p.ID_COMMODITY = k.ID_KOMODITAS
WHERE p.PRODUCTION_DATE = DATE_FORMAT('".$date."','%Y%m%d')  AND p.COMPANY_CODE='".$company."' AND p.ACTIVE=1 AND k.KODE_JENIS IN ('".$commodity_type."')";
...
(ba.BALANCE) AS BALANCE_TODAY, 
ba.BALANCE_YESTERDAY AS BALANCE_YESTERDAY, 
ba.PROCESSED_HOUR AS PROCESSED_HOUR,
(SELECT SUM(b.PROCESSED_HOUR) AS PROCESSED_HOUR_SHI FROM s_ba b WHERE b.BA_DATE BETWEEN '".$f_day."' AND ba.BA_DATE AND b.ACTIVE=1 AND b.COMPANY_CODE='".$company."') AS PROCESSED_HOUR_SHI,
