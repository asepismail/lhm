        ...
        <table width="100%" border="0" class="teks_" id="input_table" >
            ...
            <div id="tabs" style="width:98%;margin-top:25px">
                <ul>
                    ...
                    <li><a href="#tabs-2">PRODUCTION</a></li>
                    <li><a href="#tabs-3">DISPATCH</a></li>
                    <li><a href="#tabs-4">STORAGE STOCK</a></li>
                    <li><a href="#tabs-5">OTHER STOCK</a></li>                    
...
var loadView = function(){
	jGrid_va = jQuery("#list_cpo").jqGrid(
            {
                url:url+'s_input_ba/LoadData_Storage/',
                mtype : "POST",
                datatype: "json",
...
var loadView = function(){
	jGrid_va = jQuery("#list_production").jqGrid(
            {
                url:url+'s_input_ba/LoadData_Commodity/',
                mtype : "POST",
                datatype: "json",
...
var loadView = function(){
	jGrid_va = jQuery("#list_stock").jqGrid(
            {
                url:url+'s_input_ba/LoadData_OtherStock/',
                mtype : "POST",
                datatype: "json",
...
<script type="text/javascript">
/*
function deleterowprod(){            
    var postdata = {}; 
    var ids = jQuery("#list_production").getGridParam('selrow'); 
    var data = $("#list_production").getRowData(ids) ;
	//var mag = $("#txt_idNota").val();	
	if (data.ID_COMMODITY == undefined){
		alert('silakan pilih salah satu baris data yang ingin dihapus!')
	}else{
		var answer = confirm ("Hapus Detail dengan komoditas : " + data.COMMODITY + " ?" );
		if (answer){					
			$("#frm_load").dialog('open');
			var postdata = {};
			var postdata2 = {};
			postdata2['CRUD'] =  'DELDPROD';					
								
			di=0;
			dp = $("#list_production").getDataIDs();
			
			$.each(dp, function(n, rowid){ 
				var data = $("#list_production").getRowData(rowid) ; 
				di=di+1;
				postdata['ID_ANON'+i] = data.ID_ANON;
				postdata['NO_SPB'+i] = $("#txt_noSpb").val();
				postdata['AFD'+i] = data.AFD;
				postdata['BLOCK'+i] = data.BLOCK;
				postdata['JANJANG'+i] = data.JANJANG;
				postdata['TANGGAL_PANEN'+i] = data.TANGGAL_PANEN;
				postdata['TONASE'+i] =  data.JANJANG;						
			});
				...
				$.ajax({
						...
						if(obj.error==true){
							...
							alert(obj.status);															
							jQuery("#list_production").setGridParam({url:url+"s_input_ba/LoadData_Commodity"}).trigger("reloadGrid");									
							$("#frm_load").dialog('close');			
							}
						}
					}); 
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
        ...
        if (answer){
            ...
            $.ajax({
                    type:           'post',
                    cache:          false,
                    url:            url+'s_input_ba/pdf_daily/'+ dates+'/'+id_ba,
                    data:           {myJson:  data} ,
                    success: function(msg){
						var win=window.open(url+'s_input_ba/pdf_daily/'+ dates+'/'+id_ba, '_blank');
  						win.focus();
                    }
            });        
        }
    ...
    //}else{
        ...
        if (answer){
            ...
            $.ajax({
                    type:           'post',
                    cache:          false,
                    url:            url+'s_input_ba/pdf/'+ get_periode(),
                    data:           {myJson:  data} ,
                    success: function(msg){
						var win=window.open(url+'s_input_ba/pdf/'+ get_periode(), '_blank');
  						win.focus();
                    }
            });        
        //}
    ...
    var data = {
        ...
        data = JSON.stringify(data);
            $.ajax({
                    type:           'post',
                    cache:          false,
                    url:            url+'s_input_ba/xls_month/'+ get_periode(),
                    data:           {myJson:  data} ,
                    success: function(msg){
						var win=window.open(url+'s_input_ba/xls_month/'+ get_periode(), '_blank');
  						win.focus();
                    }
            });        
...
jQuery(document).ready(function(){
    ...
    $("#search_form").dialog({
        ...
        open:function(){
            jQuery("#list_ba").smartSearchPanel('#search_form', {dialog:{width: 530}},'s_input_ba/search_data');
        }
    });   
})
...
function processedAutoComplete(){
	...
	alert($("#txt_company").val())
	if($("#txt_company").val()=='MAG'|| $("#txt_company").val()=='TPAI'){
		alert(averageCageWeight); //5362.478632478633
		alert(lori_olah); //99
		alert(averageCageWeight*lori_olah); //530885.3846153846
		alert(Math.floor(averageCageWeight*lori_olah,0)); //530885
		alert(rounds(Math.floor(averageCageWeight*lori_olah,0))); // 530890
		//var ffb_processed = rounds(Math.floor(averageCageWeight*lori_olah,0));
		var ffb_processed = rounds_hundred(Math.floor(averageCageWeight*lori_olah,0));
	}else if($("#txt_company").val()=='LIH' || $("#txt_company").val()=='SMI' ){
		var ffb_processed = rounds_thousand(rounds(Math.floor(averageCageWeight,0))*lori_olah);
	}else{
		var ffb_processed = rounds_thousand(Math.floor(averageCageWeight*lori_olah,0));	
	}
	...
	if(ffb_processed==0){
		jQuery("#list_production").setGridParam({url:url+"s_input_ba/LoadNoProductionByDate"+"/"+$("#txt_tglba").val()}).trigger("reloadGrid");	
	}else{
		jQuery("#list_production").setGridParam({url:url+"s_input_ba/LoadProductionByDate"+"/"+$("#txt_tglba").val()}).trigger("reloadGrid");	
	}
}
...
function rounds_hundred($round){	
	var str = String($round);
	var digit =str.length - 2
	var right = parseFloat(str.substr(digit)); 
...
function hourAutoComplete(){
	...
	if (cbc_from!=0 && cbc_to!=0){
		...
		processedHour1 = Math.floor(processedHour);
		processedHour2 = processedHour - processedHour1;
		alert(processedHour1);
		alert(processedHour2);
		processedHour = processedHour1+(processedHour2/100*60);
		alert(processedHour);
	}else{
		processedHour = 0;
	}
...
function resetAutocomplete(){	
	 $.post(url+"s_input_ba/get_tbs/"+$("#txt_tglba").val(), function(data){
			for (var i = 0; i < data.length ; i++ ){
					 ...
					 $("#txt_group").val(group);
					 $("#txt_plasma").val(plasma);
			} 
	 });
	 
	 $.post(url+"s_input_ba/get_restan/"+$("#txt_tglba").val(), function(data){
			for (var i = 0; i < data.length ; i++ ){
					 var d = data.split('~');
					 balance = d[0];
					 $("#txt_balance_yesterday").val(balance);
			} 
	 });
	 
	 $.post(url+"s_input_ba/get_buahmentah/"+$("#txt_tglba").val()+"/"+"GRD_BUAHMENTAH", function(data){
			for (var i = 0; i < data.length ; i++ ){
					 var d = data.split('~');
					 crud = d[0];
					 $("#txt_crud").val(crud);
	 ...
	 $.post(url+"s_input_ba/get_buahmentah/"+$("#txt_tglba").val()+"/"+"GRD_BUAHBUSUK", function(data){
			for (var i = 0; i < data.length ; i++ ){
					 var d = data.split('~');
					 crud = d[0];
					 $("#txt_busuk").val(crud);
	 ...
	 $.post(url+"s_input_ba/get_buahmentah/"+$("#txt_tglba").val()+"/"+"GRD_TANGKAIPANJANG", function(data){
			for (var i = 0; i < data.length ; i++ ){
					 var d = data.split('~');
					 crud = d[0];
					 $("#txt_tangkai").val(crud);
			} 
	 });
	 
	 $.post(url+"s_input_ba/get_buahmentah/"+$("#txt_tglba").val()+"/"+"GRD_TANDANKOSONG", function(data){
			for (var i = 0; i < data.length ; i++ ){
					 var d = data.split('~');
					 crud = d[0];
					 $("#txt_jjk").val(crud);
	 ...
	 $.post(url+"s_input_ba/get_buahmentah/"+$("#txt_tglba").val()+"/"+"GRD_BRONDOLAN", function(data){
			for (var i = 0; i < data.length ; i++ ){
					 var d = data.split('~');
					 crud = d[0];
					 $("#txt_brondolan").val(crud);
			} 
	 });
	 
	 /*
	 $.post(url+"s_input_ba/get_ffb_processed/"+$("#txt_tglba").val(), function(data){
			for (var i = 0; i < data.length ; i++ ){
					 var d = data.split('~');
					 ffb_processed = d[0];
					 //$("#txt_ffb_processed").val(ffb_processed);
	 ...
	 */
	 ...
	 jQuery("#list_production").setGridParam({url:url+"s_input_ba/LoadProductionByDate"+"/"+$("#txt_tglba").val()}).trigger("reloadGrid");
	 jQuery("#list_dispatch").setGridParam({url:url+"s_input_ba/LoadDispatchByDate"+"/"+$("#txt_tglba").val()}).trigger("reloadGrid");
	 jQuery("#list_cpo").setGridParam({url:url+"s_input_ba/LoadStorageByDate"+"/"+$("#txt_tglba").val()}).trigger("reloadGrid");
	 jQuery("#list_stock").setGridParam({url:url+"s_input_ba/LoadOtherStockByDate"+"/"+$("#txt_tglba").val()}).trigger("reloadGrid");
}
</script>
...
var loadView = function()
        ...
        jGrid_va = jQuery("#list_ba").jqGrid(
            {
                url:url+'s_input_ba/LoadData/'+get_periode(),
                mtype : "POST",
                datatype: "json",
                colNames: colNamesT ,
                colModel: colModelT,
...
function crot(cl){
	...
	$.post('<?= base_url()?>index.php/s_input_ba/get_approval/', '', function(data){
            var d = data.split('~');
             alert(d[2]);
			 $("#submitdata").hide();
...
function reloadGrid(){
	 var company_code='<?php echo $company_code;?>'; 
     jQuery("#list_ba").setGridParam({url:url+'s_input_ba/LoadData/'+get_periode()}).trigger("reloadGrid");   
}
</script>
...
function input_ba(){
	...
	//jQuery("#list_production").setGridParam({url:url+"s_input_ba/LoadData_Commodity/"}).trigger("reloadGrid"); // clear grid production
	//jQuery("#list_cpo").setGridParam({url:url+"s_input_ba/LoadData_Storage/"}).trigger("reloadGrid"); // clear grid quality
	//jQuery("#list_dispatch").setGridParam({url:url+"s_input_ba/LoadData_Commodity/"}).trigger("reloadGrid"); // clear grid dispatch
	//jQuery("#list_stock").setGridParam({url:url+"s_input_ba/LoadData_OtherStock/"}).trigger("reloadGrid"); // clear grid stock
	//jQuery("#list_stock").setGridParam({url:url+"s_input_ba/LoadData_Commodity/"}).trigger("reloadGrid"); // clear grid stock
...
function edit_ba(cl){
    ...
    if (ids=="" || ids==null || ids==undefined)
    {
		...
		$("#txt_company").val(data.COMPANY_CODE);
		$("#cmd_appNote").hide();
		$("#cmd_newNote").show();
		...
		jQuery("#list_production").setGridParam({url:url+"s_input_ba/LoadDetail_Production"+"/"+data.ID_BA}).trigger("reloadGrid");
		
		jQuery("#list_dispatch").setGridParam({url:url+"s_input_ba/LoadDetail_Dispatch"+"/"+data.ID_BA}).trigger("reloadGrid");
		
		jQuery("#list_stock").setGridParam({url:url+"s_input_ba/LoadDetail_Stock"+"/"+data.ID_BA}).trigger("reloadGrid");
		
		jQuery("#list_cpo").setGridParam({url:url+"s_input_ba/LoadDetail_StorageStock"+"/"+data.ID_BA}).trigger("reloadGrid");
...
function save_ba(){
    ...
    if (answer){//start answer
		if($("#txt_sta").val()=='APPROVED'){ //start approved
			...
			postdata_id['FFB_INTI'] =  $("#txt_inti").val();
			postdata_id['FFB_PLASMA'] =  $("#txt_plasma").val();
			postdata_id['FFB_SUPPLIER'] =  $("#txt_supplier").val();
			postdata_id['FFB_GROUP'] =  $("#txt_group").val();
			postdata_id['FFB_PROCESSED'] =  $("#txt_ffb_processed").val();
			postdata_id['BALANCE_YESTERDAY'] =  $("#txt_balance_yesterday").val();
			postdata_id['LORI_OLAH'] =  $("#txt_lori_olah").val();
			postdata_id['LORI_RESTAN'] =  $("#txt_lori_restan").val();
			postdata_id['CAGE_WEIGHT'] =  $("#txt_cage_weight").val();
			
			postdata_id['BUAH_MENTAH'] =  $("#txt_crud").val();
			postdata_id['BUAH_BUSUK'] =  $("#txt_busuk").val();
			postdata_id['JJK'] =  $("#txt_jjk").val();
			postdata_id['TANGKAI'] =  $("#txt_tangkai").val();
			postdata_id['BRONDOLAN'] =  $("#txt_brondolan").val();
			postdata_id['HOUR_FROM'] =  $("#txt_hour_from").val();
			postdata_id['HOUR_TO'] =  $("#txt_hour_to").val();
			postdata_id['CBC_FROM'] =  $("#txt_cbc_from").val();
			postdata_id['CBC_TO'] =  $("#txt_cbc_to").val();
			...
			postperform['PROCESSED_HOUR'] =  $("#txt_processed").val();
			postperform['THROUGHPUT'] =  $("#txt_throughput").val();
			postperform['MILL_UTILIZATION'] =  $("#txt_mill").val();
