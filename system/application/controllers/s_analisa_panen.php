...
function generate_titip_olah_smi(){
        $company = trim(htmlentities($this->session->userdata('DCOMPANY'),ENT_QUOTES,'UTF-8'));
        $periode = trim(htmlentities($this->uri->segment(3),ENT_QUOTES,'UTF-8'));
        $periode_to = trim(htmlentities($this->uri->segment(4),ENT_QUOTES,'UTF-8'));
		$user = trim(htmlentities($this->session->userdata('LOGINID'),ENT_QUOTES,'UTF-8'));
        ...
        if(!empty($periode) && !empty($company)){
            $data_panen=$this->model_s_analisa_panen->generate_titip_olah_smi($ar,$ar2,$company, $user);
...
<tr><td  align='center' rowspan='3' class='tbl_th'> NO. </td>
	<td rowspan='3' align='center' class='tbl_th'>TANGGAL</td>
	<td colspan='9' align='center' class='tbl_th2'>SML</td>
    <td colspan='9' align='center' class='tbl_th2'></td> 
    <td colspan='9' align='center' class='tbl_th2'>SMI</td> 
 </tr>
...
<tr>
	...
	<td align='center' class='tbl_th'>Stock KERNEL</td>
			...
			if($company=='SMI' || $company=='SML' || $company=='SSS'){
				$stock_awal_cpo_sml = $this->model_s_analisa_panen->get_stock_awal($ar,'SMLSMI', 'CPO');
				//$stock_awal_cpo_sss = $this->model_s_analisa_panen->get_stock_awal($ar,'SSS', 'CPO');
				$stock_awal_cpo_gkm = $this->model_s_analisa_panen->get_stock_awal($ar,'SMI', 'CPO');
				
				$stock_awal_kernel_sml = $this->model_s_analisa_panen->get_stock_awal($ar,'SMLSMI', 'KERNEL');
				//$stock_awal_kernel_sss = $this->model_s_analisa_panen->get_stock_awal($ar,'SSS', 'KERNEL');
				$stock_awal_kernel_gkm = $this->model_s_analisa_panen->get_stock_awal($ar,'SMI', 'KERNEL');
			}					
...
WHERE ID_ANON ='". $row_tonase['ID_ANON'] ."'";
	...
	function generate_nab(){
		...
		//$array_company = array('TPAI','GKM', 'SSS', 'SML', 'ASL', 'MSS');
		//$array_company = array('LIH','TPAI');
		//$array_company = array('GKM');
			...
			if ($data_panen[0]!=NULL){
				...
				if ($status==TRUE&&$status2==TRUE){ 
					foreach($data_panen as $row){					
						$tanggal=$row['TANGGAL'];		
						$location_code = $row['LOCATION_CODE'];					
						$shi_janjang_panen = $this->model_s_analisa_panen->get_janjang_shi('20151001', $tanggal,$company,$location_code,$tabel1);			
						$shi_berat_panen =  $this->model_s_analisa_panen->get_berat_panen_shi('20151001', $tanggal,$company,$location_code, $tabel1);
						$shi_janjang_angkut =  $this->model_s_analisa_panen->get_janjang_angkut_shi('20151001', $tanggal,$company,$location_code);
						$shi_berat_angkut =  $this->model_s_analisa_panen->get_berat_angkut_shi('20151001', $tanggal,$company,$location_code);	
						$restan = $this->model_s_analisa_panen->get_restan('20151001', $tanggal,$company,$location_code,$tabel1);
						...
						if ($company=='MAG' || $company=='LIH' || $company=='TPAI' || $company=='MSS' || $company=='ASL' || $company=='SSS'){										
