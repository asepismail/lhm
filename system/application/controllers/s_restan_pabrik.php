<?php
class s_restan_pabrik extends Controller{
    private $data;
    function __construct(){
        parent::__construct();
        $this->load->model('model_s_restan_pabrik');
        $this->load->model('model_c_user_auth');  
        
        $this->load->library('form_validation');
        
        $this->lastmenu="s_restan_pabrik";
        $this->data = array();    
    }
    
    function index(){
        $view="info_s_restan_pabrik";
        
        //$data = array();
        $this->data['judul_header'] = "Data Restan (Pencatatan Restan Pabrik)";
        $this->data['js'] = "";
    ...
    function LoadData(){
        ...
        echo json_encode($this->model_s_restan_pabrik->LoadData($periode,$company));   
    }
    ...
    function search_data(){
        ...
        echo json_encode($this->model_s_restan_pabrik->data_search($data['rules'], $company));    
    } 
    ...
    function add_new($data_id){
        ...
        $data_post['ID_RESTAN'] = trim($this->global_func->createMy_ID('s_restan','ID_RESTAN',$company."RST","TANGGAL",$company));
        $data_post['TANGGAL'] = strtoupper(trim(htmlentities($data_id['TANGGAL'],ENT_QUOTES,'UTF-8'))) ;
        $data_post['RESTAN']=strtoupper(trim(htmlentities($data_id['RESTAN'],ENT_QUOTES,'UTF-8'))) ;

        $data_post['INPUT_BY'] = trim(htmlentities($this->session->userdata('LOGINID'),ENT_QUOTES,'UTF-8')); 
        $data_post['COMPANY_CODE'] = $company;
        
        $validate_numeric=$this->validate_numeric($data_post['RESTAN']);
        if( strtolower($validate_numeric)=='false'){
            $return['status'] ="Nilai RESTAN harus angka";
            $return['error']=true;        
        }
        
        if (empty($data_post['ID_RESTAN']) || trim($data_post['ID_RESTAN'])==''){
            $return['status'] = "Harap isi ID_RESTAN";
            $return['error']=true; 
              
        }elseif(strlen($data_post['ID_RESTAN']) > 50){
            $return['status']  ="Panjang karakter ID_RESTAN melebihi batas maksimal";
            $return['error']=true;
        }
        
        if (empty($data_post['RESTAN']) || trim($data_post['RESTAN'])==''){
            $return['status'] = "Harap isi RESTAN";
            $return['error']=true;   
        }elseif(strlen($data_id['RESTAN']) > 50){
            $return['status']  ="Panjang karakter RESTAN melebihi batas maksimal";
            $return['error']=true;
        }
        ...
        if(empty($return['status']) && $return['error']===false){     
            $insert_id = $this->model_s_restan_pabrik->add_new($company,$data_post);
    ...
    function update_data($data_id){
        ...
        $id_restan = strtoupper(trim(htmlentities($data_id['ID_RESTAN'],ENT_QUOTES,'UTF-8'))) ;
        ...
        if(empty($return['status']) && $return['error']===false){     
            $update_id = $this->model_s_restan_pabrik->update_restan($id_restan,$data_post,$company);
    ...
    function delete_data($data_id){
        ...
        if (empty($id_restan) || trim($id_restan)==='' || $id_restan===false){
            $return['status']="ID_RESTAN KOSONG !!";
            $return['error']=true;   
        }elseif(strlen($id_restan) > 50){
        ...
        if(empty($return['status']) && $return['error']===false){     
            $delete_id = $this->model_s_restan_pabrik->delete_restan($id_restan,$company);
