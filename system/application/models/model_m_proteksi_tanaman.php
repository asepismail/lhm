<?php
class model_m_proteksi_tanaman extends Model
{
    ...
    function LoadData($bulan,$tahun,$company,$afd='-',$block='-'){
        ...
        $queries = "SELECT * FROM s_proteksi_tanaman WHERE ACTIVE=1 AND COMPANY_CODE='".$company."' 
        ...
        foreach($objects as $obj)
        {
            $cell = array();
            array_push($cell, $no); 
            array_push($cell, htmlentities($obj->ID_PROTEKSI_TANAMAN,ENT_QUOTES,'UTF-8'));
            ...
            array_push($cell, htmlentities($obj->KONDISI_HAMA,ENT_QUOTES,'UTF-8'));
			array_push($cell, htmlentities($obj->KONDISI_PENYAKIT,ENT_QUOTES,'UTF-8'));
			array_push($cell, htmlentities($obj->TANAMAN_INANG,ENT_QUOTES,'UTF-8'));
			array_push($cell, htmlentities($obj->BURUNG_HANTU,ENT_QUOTES,'UTF-8'));
    ...
    function data_search($data_search, $company){
        ...
        $queries = "SELECT * FROM s_proteksi_tanaman ". $where;
    ...
    ##################################################################################*/
	/*
    function set_proteksi_tanaman_periode($periode,$company){
        $periode = $this->db->escape_str($periode);
        $company = $this->db->escape_str($company);
        
        $query = "SELECT PERIODE FROM s_proteksi_tanaman_periode WHERE COMPANY_CODE='".$company."'";
        ...
        if($sQuery->num_rows() > 0){
            ...
            $this->db->update('s_proteksi_tanaman_periode',$data);
            if($this->db->trans_status() === FALSE){
             ...
             $this->db->insert('s_proteksi_tanaman_periode');
             if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
	...
	*/
    /*
    function get_proteksi_tanaman_periode($company){
        $company = $this->db->escape_str($company);
        $query="SELECT PERIODE_USED FROM s_data_proteksi_tanaman WHERE COMPANY_CODE ='".$company."'";
        $sQuery = $this->db->query($query);
        $value='';
        if($sQuery->num_rows() > 0){
            $row = $sQuery->row();
            ...
            if(!empty($row->PERIODE_USED) || $row->PERIODE_USED!=null){
                ...
                $value = '<span style="color: #FF0000; font-weight: bold;"><em>proteksi_tanaman NOT SET</em></span>';   
            }
        }else{
            $value = '<span style="color: #FF0000; font-weight: bold;"><em>proteksi_tanaman NOT SET</em></span>';   
        } 
	...
	*/
    /*########################## END PENETAPAN PERIODE proteksi_tanaman ###############################
    ##################################################################################*/
    ...
    function add_new($company, $data_post){
        ...
        if( isset($company))
        {
            if(!empty($company))
            {
                $cek_data_exist = $this->cek_data_exist('s_proteksi_tanaman',array('AFD'=>$data_post['AFD'],'BLOK_TANAH'=>$data_post['BLOK_TANAH'], 'BULAN'=>$data_post['BULAN'],'TAHUN'=>$data_post['TAHUN'], 'COMPANY_CODE'=>$company,'ACTIVE'=>1),'ID_PROTEKSI_TANAMAN'); 
				 
				 
                if ($cek_data_exist <= 0){
                    $this->db->insert( 's_proteksi_tanaman', $data_post ); 
    ...
    function update_proteksi_tanaman($data_post, $company){
		$company = trim($this->db->escape_str($company));
        $status=FALSE;
        ...
        if(empty($status) || $status==false){
            ...
            $this->db->update('s_proteksi_tanaman',$data_post);
            if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
	...
	/*
		get_active_proteksi_tanaman added by Asep, 20130507		
	*/
	/*
	function get_active_proteksi_tanaman($company, $block, $tgl){
        $company = $this->db->escape_str($company);
        ...
        $query="SELECT bj.AFD,bj.BLOCK,bj.VALUE,
				...
				JOIN (
						...
						GROUP BY BLOCK 
				) proteksi_tanaman ON proteksi_tanaman.AFD = bj.AFD AND proteksi_tanaman.BLOCK = bj.BLOCK 
								AND proteksi_tanaman.MAX_PERIODE = bj.PERIODE
WHERE bj.BLOCK = '".$block."'";

		//$query="SELECT proteksi_tanaman.VALUE FROM s_data_proteksi_tanaman proteksi_tanaman WHERE proteksi_tanaman.BLOCK = '".$block."'  AND proteksi_tanaman.ACTIVE = 1 AND proteksi_tanaman.COMPANY_CODE ='".$company."'";
        $sQuery = $this->db->query($query);
        $value='';
        if($sQuery->num_rows() > 0){
            $row = $sQuery->row();            
	...
	*/
	function get_proteksi_tanaman_blok($company, $block, $tgl){
        $company = $this->db->escape_str($company);
		
        $query="SELECT * FROM s_proteksi_tanaman WHERE COMPANY_CODE='".$company."' AND CONCAT(TAHUN,BULAN) = DATE_FORMAT('".$tgl."','%Y%m') AND ACTIVE=1 AND BLOK_TANAH = '".$block."'";
	...
	function delete_all_data($data_post){
		...
		foreach($data_post as $keys => $vals){
			$this->db->where('ID_PROTEKSI_TANAMAN',$vals['ID_PROTEKSI_TANAMAN']);
			...
			$this->db->update('s_proteksi_tanaman',$data_post[$keys]);
    ...
    function delete_proteksi_tanaman($afd,$block,$bulan,$tahun,$company){
        $afd = $this->db->escape_str($afd);
        ...
        $status=FALSE;
        $cek_data = $this->cek_data_exist('s_proteksi_tanaman',
                    array('AFD'=>$afd,'BLOK_TANAH'=>$block,'BULAN'=>$bulan,'TAHUN'=>$tahun,
                            'COMPANY_CODE'=>$company),
                    'ID_PROTEKSI_TANAMAN');
        if ($cek_data <= 0){
            $status='Data proteksi_tanaman tidak ada di database';
        }
        ...
        if(empty($status) || $status==false){
            ...
            $this->db->set($set);
            $this->db->update( 's_proteksi_tanaman');
            
            //$this->db->delete('s_proteksi_tanaman');
            if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
...
,rawat.KONDISI_HAMA, rawat.KONDISI_PENYAKIT, rawat.TANAMAN_INANG, rawat.BURUNG_HANTU
FROM (	
...
LEFT JOIN (
	SELECT * FROM s_proteksi_tanaman r 
