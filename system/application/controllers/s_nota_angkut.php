<?php
class s_nota_angkut extends Controller
{
    private $lastmenu; 
    function __construct()
    {
        parent::__construct();
        $this->load->model('model_s_nota_angkut');
        ...
        $this->lastmenu="s_nota_angkut";   
    }
    
    function index()
    {
        //$this->output->cache(3);
        $view="info_s_nota_angkut";
    ...
    function search_data(){
        ...
        echo json_encode($this->model_s_nota_angkut->data_search($data['rules'], $company));    
    }
    ...
    function LoadData(){
        ...
        echo json_encode($this->model_s_nota_angkut->LoadData($company,$periode));   
    }
    ...
    function LoadData_Detail(){
        ...
        echo json_encode($this->model_s_nota_angkut->LoadData_Detail($company,$id_nota));   
    }
    
    function get_afdeling(){
        $company = htmlentities($this->session->userdata('DCOMPANY'),ENT_QUOTES,'UTF-8');
        $q = htmlentities($_REQUEST['q'],ENT_QUOTES,'UTF-8');
        $data_afd = $this->model_s_nota_angkut->get_afdeling($company,$q);
    ...
    function get_block(){
        ...
        $data_afd = $this->model_s_nota_angkut->get_block($company,$location_left,$q);
    ...
    function create_nota(){
	 ...
	 $no_spb = str_replace(" ","",strtoupper(trim(htmlentities($data_id['NO_SPB'],ENT_QUOTES,'UTF-8'))));
        ...
        if(empty($TGL_AKTIVITAS) || $TGL_AKTIVITAS==null || $TGL_AKTIVITAS==false){
			...
			// validasi closing
			$isClose = $this->model_s_nota_angkut->get_status_close($company, $TGL_AKTIVITAS);
			if ($isClose == 1){
	...
	//var_dump($data_detail);
        foreach($data_detail as $key => $val)
        {
			...
			//}elseif(strtoupper(trim(substr($key,0,strlen(trim($key))-1)))=='BLOCK') {
		      ...
		      break;  
                    ...
                    $lc = trim($val);  
                    $afd = $tmp_afd;
                    $data_lokasi = $this->model_s_nota_angkut->lokasi_validate($afd,$lc,$company);   
            ...
            //}elseif(strtoupper(trim(substr($key,0,strlen(trim($key))-1)))=='NO_SPB') {
				...
				if($company<>'NAK'){
					$tgl_lokasi = $this->model_s_nota_angkut->lokasitgl_validate($date,$block_panen,$company); 
		...
		if(empty($return['status']) && $return['error'] == false){ 
            		foreach($data_post_d as $val){	
				if ($val['TONASE']==0){
					$return['status']="Proporsi tonase blok gagal terbentuk, silakan klik ulang tombol simpan";
					$return['error']=true;						
				}
			}
		}
        ...
        if(empty($return['status']) && $return['error'] == false){   
            ...
            $insert_id = $this->model_s_nota_angkut->add_new($id, $company, $data_post, $flag_wp );
			
			if($insert_id['error'] == false){ 
            	$insert_detail = $this->model_s_nota_angkut->add_new_detail($data_post['ID_NT_AB'], $no_spb, $company, $data_post_d);
    ...
    function update_nota(){
        ...
        $no_spb = trim(htmlentities($data_id['NO_SPB'],ENT_QUOTES,'UTF-8'));
        $no_tiket = $no_spb;
        $cek_status = $this->model_s_nota_angkut->cek_nota_status($no_tiket,$id_nota);
        if($cek_status == 1 || $cek_status =='1') 
        {
		 ...
		 if(empty($return['status']) && $return['error'] == false){ 		
            ...
            if(empty($return['status']) && $return['error']==false){ 
                $update_id = $this->model_s_nota_angkut->update_data($id_nota,$no_tiket, $company, $data_post );				
                if($update_id['error'] == false){ 
					$update_detail = $this->model_s_nota_angkut->update_detail($id_nota,$no_tiket, $company, $data_post_d) ;
    ...
    function delete_nota($data_id){
       ...
       // $data_id = $data["id"];
        ...
        if(empty($return['status']) && $return['error']==false){     
            $delete_id = $this->model_s_nota_angkut->delete_nota($id_nota,$plat_no,$no_spb,$company);
    ...
    function delete_notadetail($data_id){
        ...
        if(empty($return['status']) && $return['error']==false){     
            $delete_id = $this->model_s_nota_angkut->delete_notadetail($id_anon,$id_nt_ab);
            $return['status']=  $delete_id;
            $return['error']=false;
            echo json_encode($return);
    ...
    function get_no_kendaraan(){
        ...
        $company = htmlentities($this->session->userdata('DCOMPANY'),ENT_QUOTES,'UTF-8');
        $data_kendaraan = $this->model_s_nota_angkut->get_no_kendaraan($q,$company);
        //echo $q;
        $kendaraan = array();
	...
	function get_supplier(){
        ...
        $company = htmlentities($this->session->userdata('DCOMPANY'),ENT_QUOTES,'UTF-8');
        $data_supplier = $this->model_s_nota_angkut->get_supplier($supplier,$company);
        //echo $q;
        $supplier = array();
    ...
    function get_no_spb(){
        ...
        $data_spb = $this->model_s_nota_angkut->get_no_spb($q,$company,$tanggalm);
        //echo $q;
        $spb = array();
   ...
   function search_spb(){
        ...
        echo json_encode($this->model_s_nota_angkut->search_spb($spb, $no_kend,$periode, $company));
    } 
}
?>
