<?php
class model_s_restan_pabrik extends Model{
    ...
    function LoadData($periode,$company){
        ...
        $queries ="SELECT ID_RESTAN, TANGGAL, RESTAN, COMPANY_CODE FROM s_restan 
        ...
        foreach($objects as $obj)
        {
            $cell = array();
            array_push($cell, $no);
            array_push($cell, htmlentities($obj->ID_RESTAN,ENT_QUOTES,'UTF-8'));
            array_push($cell, htmlentities($obj->TANGGAL,ENT_QUOTES,'UTF-8')); 
            array_push($cell, htmlentities($obj->RESTAN,ENT_QUOTES,'UTF-8'));
    ...
    function data_search($data_search,$company){
        ...
        $queries ="SELECT * FROM s_restan ".$where;
    ...
    function add_new($company, $data_post){
        ...
        $cek_data_exist = $this->cek_data_exist('s_restan',
                    array('ID_RESTAN'=>$data_post['ID_RESTAN'],'TANGGAL'=>$data_post['TANGGAL'],'COMPANY_CODE'=>$company),'ID_RESTAN');
        if ($cek_data_exist > 0){
            $status='Data Input ID telah ada di database';
        }
        
        unset($cek_data_exist);
        $cek_data_exist = $this->cek_data_exist('s_restan',array('ACTIVE'=>1,'TANGGAL'=>$data_post['TANGGAL'],'COMPANY_CODE'=>$company),'ID_RESTAN');
        if ($cek_data_exist > 0){
            $status="Data Restan hari ini telah diinput !!";
        }
        
        if(empty($status) || $status===FALSE){
            $this->db->insert( 's_restan', $data_post );
    ...
    function update_restan($id_restan,$data_post,$company){
        ...
        $cek_data_exist = $this->cek_data_exist('s_restan',array('ID_RESTAN'=>$id_restan),'ID_RESTAN');
        if ($cek_data_exist <= 0){
        ...
        if(empty($status) || $status==false){
            ...
            $this->db->where('ID_RESTAN',$id_restan);
            $this->db->where('COMPANY_CODE',$company);
            $this->db->update('s_restan',$data_post);
            if($this->db->trans_status() === FALSE){
                $status = $this->db->_error_message();//"Error in Transactions!!";
            }else{
