<?php
class s_catat_ffa extends Controller{
    private $lastmenu;
    private $data;
    function __construct(){
        parent::__construct();
        $this->load->model('model_s_catat_ffa');
        $this->load->model('model_c_user_auth');  
        
        $this->load->library('form_validation');
        
        $this->lastmenu="s_catat_ffa";
        $this->data = array(); 
    }
    
    function index(){
        $view="info_s_catat_ffa";
        
        //$data = array();
        $this->data['judul_header'] = "Pencatatan FFA - Storage";
        $this->data['js'] = "";
    ...
    function LoadData(){
        ...
        $kode_storage = htmlentities($this->uri->segment(3),ENT_QUOTES,'UTF-8');
        $periode = htmlentities($this->uri->segment(4),ENT_QUOTES,'UTF-8');
        
        echo json_encode($this->model_s_catat_ffa->LoadData($kode_storage,$periode,$company));   
    }
    
    function search_data(){
        $company = htmlentities($this->session->userdata('DCOMPANY'),ENT_QUOTES,'UTF-8');
        $data = json_decode($this->input->post('filters'), true);
        echo json_encode($this->model_s_catat_ffa->data_search($data['rules'], $company));  
    }
    ...
    function add_new($data_id){
        ...
        $data_post['ID_FFA'] = trim($this->global_func->createMy_ID('s_ffa','ID_FFA',$company."FFA","DATE",$company));
        ...
        if (empty($data_post['ID_FFA'])){
            $status = "Harap isi ID_FFA";
            $return['status']=$status;
            $return['error']=true;   
        }elseif(strlen($data_post['ID_FFA']) > 50){
            $status  ="Panjang karakter ID_FFA melebihi batas maksimal";
            $return['status']=$status;
            $return['error']=true;
        }
        
        if (empty($data_post['ID_STORAGE'])){
            $status = "Harap isi ID STORAGE"; 
            $return['status']=$status;
            $return['error']=true;  
        }elseif(strlen($data_id['ID_STORAGE']) > 50){
            $status  ="Panjang karakter ID_STORAGE melebihi batas maksimal";
            $return['status']=$status;
            $return['error']=true;
        }
        
        if (empty($data_post['FFA'])){
        ...
        if(empty($status)){     
            $insert_id = $this->model_s_catat_ffa->add_new($company,$data_post);
    ...
    function update_data($data_id){
        ...
        $id_ffa=strtoupper(trim(htmlentities($data_id['ID_FFA'],ENT_QUOTES,'UTF-8'))) ;
        if (empty($id_ffa)){
            $status = "Harap isi ID_FFA";
            $return['status']=$status;
            $return['error']=true;
        ...
        if(empty($return['status']) && $return['error']==false){    
            $update_id = $this->model_s_catat_ffa->update_ffa($id_ffa,$data_post,$company);
    ...
    function delete_data($data_id){
        ...
        if(empty($status)){     
            $delete_id = $this->model_s_catat_ffa->delete_ffa($id_ffa,$company);
    ...
    function get_storage(){
        ...
        $company = htmlentities($this->session->userdata('DCOMPANY'),ENT_QUOTES,'UTF-8');
        $data_storage = $this->model_s_catat_ffa->get_storage($q,$company);
        //echo $q;
        $storage = array();
