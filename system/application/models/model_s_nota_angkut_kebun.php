<?php
class model_s_nota_angkut_kebun extends Model{
...
ORDER BY m_gang_activity_detail.LOCATION_CODE";			
		...
		*/
		...
		//$flag_wb == "0" => perusahaan tidak ada timbangan
	 	if ($flag_wb == "1"){ 
			...
			if ($cek_data_exist <= 0){
				//$status='Data SPB belum ada di database = '.$data_post['NO_SPB'];
    ...
    function add_new_detail($id, $no_spb, $company, $data_post){
        ...
        if(isset($id) && isset($company)){
			...
			$this->round_tonase_kebun($no_spb);
			$this->round_tonase($no_spb);
        }else{
    ...
    function update_detail($id,$no_tiket,$company,$data){
		...
		if(empty($return['status']) && $return['error'] == false){  
            ...
            if($notastatus!=1){
                ...
                //}
				$this->round_tonase_kebun($no_tiket);
				$this->round_tonase($no_tiket);
            }else{
                $status="Data Tidak dapat di Update..!!"."\n";  
				$return['status']=$status;
        		$return['error']=true;
	...
	*/
	...
	/*
		get_status_close added by Asep, 20130507		
	*/
	function get_netto($no_spb){
        $no_spb = $this->db->escape_str($no_spb);
		
        $query="SELECT BERAT_BERSIH FROM s_data_timbangan_kebun WHERE NO_SPB = '".$no_spb."';";
	...
	function round_tonase_kebun($id_nab){
		$i=0;		
		$sisa=0;
		$round_tonase=0;
		$qNab= "SELECT nabd.ID_ANON, nabd.BLOCK, nabd.TONASE_KEBUN, ROUND(nabd.TONASE_KEBUN) AS ROUND_TONASE, (nabd.TONASE_KEBUN - ROUND(nabd.TONASE_KEBUN)) AS SISA, timbang.BERAT_BERSIH FROM s_nota_angkutbuah nab 
		...
		if($i > 0){
			foreach ($dataNab->result_array() as $row_nab){				
				...
				$i=$i-1;								
				$sUpdateDetail="UPDATE s_nota_angkutbuah_detail SET ROUND_TONASE_KEBUN = ". $round_tonase ." WHERE ID_ANON ='". $row_nab['ID_ANON'] ."'";
				$this->db->reconnect();
