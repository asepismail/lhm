...
class model_s_catat_sounding extends Model{
    ...
    function add_new($company, $data_post){
        ...
        if(empty($status) || $status===FALSE){
            ...
            if($this->db->trans_status() === FALSE){
                ...
                $this->calc_weight($data_post['TEMPERATURE'],$data_post['HEIGHT'],$data_post['ID_SOUNDING'],$data_post['ID_STORAGE'],$company, $data_post['WEIGHT']); //hitung berat tangki  
            }
        }
        return $status;
    }
    ...
    function update_sounding($id_sounding,$data_post,$company){
        ...
        if(empty($status) || $status===FALSE){
            ...
            if($this->db->trans_status() === FALSE){
                ...
                $this->calc_weight($data_post['TEMPERATURE'],$data_post['HEIGHT'],$id_sounding,$data_post['ID_STORAGE'],$company, $data_post['WEIGHT']);
    ...
    function calc_volume($height,$id_sounding,$id_storage,$company){
        ...
        $volume=0;
        if($company == 'LIH' || $company == 'GKM' || $company == 'SMI' || $company == 'NRP'){
    ...
    function calc_weight($temp,$height,$id_sounding,$id_storage,$company,$weight){ //TONASE
		...
		if (!($height==0 || empty($height))){
			...
			if($company=='LIH'){
				...
				$tonase=$volume_3*$bj;   
			}elseif($company=='GKM' || $company=='SMI' || $company=='NRP' || $company=='TPAI'){
				$hi=$height*1000;
				if ($hi>24 && $hi<100){
