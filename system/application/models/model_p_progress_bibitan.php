...
class model_p_progress_bibitan extends Model 
{

    function model_p_progress_bibitan()
    {
        parent::Model(); 
	...
	function insert_p_progress_bibitan ( $data )
	{
		$this->db->insert( 'p_progress_bibitan', $data );
		
		return $this->db->insert_id();   
	}
	
	function update_p_progress_bibitan ( $tgl_progress,$activity,$location,$company, $data )
	{
		$this->db->where('TGL_PROGRESS',$tgl_progress);
		$this->db->where('ACTIVITY_CODE',$activity);
		$this->db->where('LOCATION_CODE',$location);
		$this->db->where('COMPANY_CODE',$company);  
		$this->db->update( 'p_progress_bibitan', $data );   
	}
	
	function delete_p_progress_bibitan ( $id, $company)
	{
		$this->db->where( 'ID_PROGRESS_BIBITAN', $id );  	
		$this->db->where( 'COMPANY_CODE', $company );  	
		$this->db->delete('p_progress_bibitan');   
	}
...
AND lhm.ACTIVITY_CODE IN (SELECT ACCOUNTCODE FROM m_progress_map WHERE PENGGUNAAN = 'NS' AND PARENT = 0) 
...
LEFT JOIN m_progress_map pm ON (pm.ACCOUNTCODE = lhm.ACTIVITY_CODE)
WHERE lhm.COMPANY_CODE = '".$company."' AND DATE_FORMAT(lhm.LHM_DATE,'%Y%m%d') = '".$tgl."'
AND ACTIVITY_CODE IN (SELECT ACCOUNTCODE FROM m_progress_map WHERE PENGGUNAAN = 'NS' AND PARENT = 0) 
GROUP BY lhm.LHM_DATE, lhm.LOCATION_CODE, lhm.ACTIVITY_CODE
ORDER BY ".$sidx." ".$sord.""; 

		$objects = $this->db->query($sql,FALSE)->result(); 
        $rows =  array();
		$ID_PROGRESS_BIBITAN = 1;
		$REALISASI_UNIT = "";
		$HASIL_KERJA = "";
		foreach($objects as $obj)
        {
            $cell = array();
				...
				array_push($cell, $ID_PROGRESS_BIBITAN);
	...
	function cek_pb($tgl, $lc, $company){
		$query = $this->db->query("select count(*) as jumlah FROM p_progress_bibitan where DATE_FORMAT(TGL_PROGRESS, '%Y%m%d') = '".$tgl."' and COMPANY_CODE = '".$company."'");
	...
	function read_exist_act($tgl,$lc, $company)
    {
		...
		$sql2 = "SELECT ID_PROGRESS_BIBITAN, TGL_PROGRESS, LOCATION_CODE, ACTIVITY_CODE, ACTIVITY_DESC, SATUAN, HASIL_KERJA, COMPANY_CODE FROM p_progress_bibitan
WHERE DATE_FORMAT(TGL_PROGRESS, '%Y%m%d') = '".$tgl."' AND COMPANY_CODE = '".$company."'";
		...
		$sql = "SELECT ID_PROGRESS_BIBITAN, TGL_PROGRESS, LOCATION_CODE, ACTIVITY_CODE, ACTIVITY_DESC, SATUAN, 
	HASIL_KERJA, COMPANY_CODE  FROM p_progress_bibitan
WHERE DATE_FORMAT(TGL_PROGRESS, '%Y%m%d') = '".$tgl."' AND COMPANY_CODE = '".$company."' 
ORDER BY ".$sidx." ".$sord.""; 
		...
		foreach($objects as $obj)
        {
            $cell = array();
				...
				array_push($cell, $obj->ID_PROGRESS_BIBITAN);
	...
	function act($q, $tgl, $company){
		...
		$query = $this->db->query("SELECT LOCATION_CODE, ACTIVITY_CODE, m_coa.COA_DESCRIPTION AS DESCR, m_progress_map.UNIT1 AS UNIT1  FROM m_gang_activity_detail 
			...
			AND LEFT(m_gang_activity_detail.ACTIVITY_CODE,4) IN ('8302','8303') 
